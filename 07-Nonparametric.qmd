...
Chapter 3 introduced the shape of the chi-square distribution: always non-negative, right-skewed, naturally one-tailed, with its exact shape depending on degrees of freedom. Here's what that rejection region looks like in practice, using the goodness-of-fit example below (one of the two chi-square tests you'll meet in this section; more on what "goodness-of-fit" means right before we run it), which has $df = 3$. The critical value at $\alpha = .05$ is `r round(qchisq(.95, 3), 2)`; any statistic beyond that point (the shaded region) occurs less than 5% of the time by chance.
...
For **categorical** data, the **chi-square tests** either 1) check whether one variable's observed counts match an expected pattern (goodness-of-fit) or 2) whether two categorical variables are related (test of independence). These use the chi-square distribution introduced back in Chapter 3.

Across every test in this chapter, the testing logic is the same from Chapter 6: state $H_0$ and $H_A$, compute a statistic, and ask how surprising that statistic would be if $H_0$ were true. 
