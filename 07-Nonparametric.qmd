...
# Critical values of a discrete statistic: the most extreme values whose tail
# share is at most .025. Taken from the exact null (every sign pattern or every
# split, which the simulation approximates). A raw quantile() can return a value
# whose tail share is just over .025, putting a non-significant result inside
# the region.
exact_crit <- function(null) {
  v <- sort(unique(null))
  lo <- max(v[sapply(v, function(x) mean(null <= x)) <= .025])
  hi <- min(v[sapply(v, function(x) mean(null >= x)) <= .025])
  # each boundary sits halfway between the critical value and the next value inward
  c(lo = lo, hi = hi, lo_edge = (lo + min(v[v > lo])) / 2, hi_edge = (hi + max(v[v < hi])) / 2)
}

# Simulated null histogram: sky bars, red on the part of each bar in the rejection
# region, full-height labeled critical lines, observed statistic green dotted.
null_hist <- function(sim, exact, obs, stat, xlab) {
  cr <- exact_crit(exact)
  d <- tibble(x = sim, region = sim <= cr["lo"] | sim >= cr["hi"])
  top <- max(hist(sim, breaks = seq(cr["hi_edge"] - 2 * ceiling((cr["hi_edge"] - min(sim)) / 2) - 2,
                                    max(sim) + 2, 2), plot = FALSE)$counts)
  ggplot(d, aes(x = x, fill = region, alpha = region)) +
    geom_histogram(binwidth = 2, boundary = cr["hi_edge"], closed = "left", colour = "white") +
    scale_fill_manual(values = c(`FALSE` = "#008cff", `TRUE` = "red"), guide = "none") +
    scale_alpha_manual(values = c(`FALSE` = 1, `TRUE` = .35), guide = "none") +
    annotate("segment", x = cr[c("lo_edge", "hi_edge")], xend = cr[c("lo_edge", "hi_edge")],
             y = 0, yend = top * 1.08, colour = "red", linewidth = 0.8) +
    annotate("text", x = cr[c("lo_edge", "hi_edge")], y = top * 1.13,
             label = paste0(c("lower crit = ", "upper crit = "), cr[c("lo", "hi")]),
             colour = "red", size = 3.8) +
    annotate("segment", x = obs, xend = obs, y = 0, yend = top * 0.9,
             colour = "#1E9E1E", linetype = "dotted", linewidth = 1.4) +
    annotate("text", x = obs - 0.8, y = top * 0.95, label = paste0("observed\n", stat, " = ", obs),
             colour = "#1E9E1E", hjust = 1, size = 3.6, lineheight = 0.9) +
    labs(x = xlab, y = "Count out of 20,000 simulations")
}
```
...
#| fig-alt: "Histogram of the change in soil moisture for 12 plots. Three plots changed by minus 1 point and three by 0; the other six changed by 3, 4, 6, 6, 7 and 8 points, so the positive side stretches much further than the negative side."

ggplot(tibble(differences = differences), aes(x = differences)) +
  geom_histogram(binwidth = 1, color = "white", fill = "#008cff", boundary = -0.5) +
...
The Shapiro-Wilk test agrees with the picture ($p=.039$). Three plots didn't change at all, three dropped slightly, and the rest jumped up by quite a lot; that's not symmetric, let alone normal. The $t$-test's "significant" result was built on an assumption the data don't meet. Time for the **Wilcoxon signed-rank test**.
...
**How the null distribution of $V$ is actually built.** Under $H_0$, each plot's rank has an independent 50/50 chance of ending up positive or negative, purely by chance. That gives you $2^n$ equally likely sign patterns for $n$ nonzero differences (here, $n=9$, not 12, since the zeros already dropped out), and summing the positive ranks for every one of those patterns builds the null distribution of $V$, not just a raw count. It has a known mean and variance:
...
#| fig-cap: "Simulated null distribution of the Wilcoxon signed-rank statistic V, built from 20,000 random sign-flips of the 9 nonzero ranked differences. Shaded regions mark the two-tailed 5% rejection region; our observed V = 39 lands just outside the upper boundary."
#| fig-alt: "Histogram of 20,000 simulated values of V, roughly bell-shaped and centered near 22. Bars at or below 5 and at or above 40 are shaded red, with red vertical lines labeled lower crit equals 5 and upper crit equals 40. A green dotted line marks the observed V of 39, just to the left of the upper critical line."
...
exact_V <- apply(expand.grid(rep(list(0:1), length(ranks_abs))), 1,
                 function(s) sum(ranks_abs[s == 1]))   # all 2^9 sign patterns
obs_V <- as.numeric(wilcox_out$statistic)
null_hist(sim_V, exact_V, obs_V, "V", "V (sum of positive ranks)")
```

Our observed $V=39$ lands on the upper boundary but not inside the 5% region. $V$ can only take certain values, so the boundary can't fall exactly at 2.5%: a $V$ of 39 or more comes up just over 2.5% of the time under $H_0$, which puts the two-tailed $p$ just over .05. Not significant, but only barely.
...
#| fig-alt: "Study design schematic. IV: restoration status, categorical. Two boxes labeled Restored and Unrestored, each with n = 8 plots. DV: plant species richness, a count."
#| fig-asp: 0.34
#| out-width: "100%"
...
#| fig-alt: "Box plots of plant species richness for 8 restored and 8 unrestored plots, with each plot drawn as a point. Restored plots mostly fall between 16 and 22, with one plot at 34 circled in red far above the rest. Unrestored plots fall between 10 and 18."
...
ggplot(plot_df, aes(x = x_jit, y = Richness)) +
  ...
  geom_point(size = 2.5, color = "#10377f") +
...
#| fig-alt: "Histogram of 20,000 simulated values of U, roughly bell-shaped and centered near 32. Bars at or below 13.5 and at or above 50.5 are shaded red, with red vertical lines labeled lower crit and upper crit. A green dotted line marks the observed U of 50, just to the left of the upper critical line."
...
exact_U <- apply(combn(N, n_a), 2, function(i)
  sum(ranks_pooled[i]) - n_a * (n_a + 1) / 2)          # all choose(16, 8) splits
obs_U <- as.numeric(wilcox.test(restored, unrestored)$statistic)
null_hist(sim_U, exact_U, obs_U, "U", "U (Mann-Whitney statistic)")
```
...
Chapter 3 introduced the shape of the chi-square distribution: always non-negative, right-skewed, naturally one-tailed, with its exact shape depending on degrees of freedom. Only the upper tail counts against $H_0$: $\chi^2$ adds up squared gaps between observed and expected, so a mismatch in either direction makes it bigger, and a $\chi^2$ near 0 just means the counts match the expectation closely. @fig-7chisq-crit shows what the rejection region looks like in practice, using the goodness-of-fit example we'll run below, which has $df = 3$. The critical value at $\alpha = .05$ is `r round(qchisq(.95, 3), 2)`, and any statistic beyond that point (the shaded region) occurs less than 5% of the time by chance.

```{r}
#| label: fig-7chisq-crit
#| fig-cap: "The chi-square distribution with df = 3. The shaded region marks the rejection region at alpha = .05 (critical value 7.81)."
#| fig-alt: "A blue dot-dash chi-square curve with 3 degrees of freedom, rising steeply from zero to a peak near 1 and then trailing off to the right. A red vertical line labeled crit equals 7.81 marks the critical value, and the thin tail to its right is shaded red."
...
ggplot(df3, aes(x, density)) +
  geom_area(data = filter(df3, x >= crit), fill = "red", alpha = 0.18) +
  geom_line(colour = "blue", linetype = "dotdash", linewidth = 1.1) +
  annotate("segment", x = crit, xend = crit, y = 0, yend = max(df3$density) * 1.02,
           colour = "red", linewidth = 0.8) +
  annotate("text", x = crit, y = max(df3$density) * 1.07,
           label = paste0("crit = ", round(crit, 2)), colour = "red", size = 4.2) +
...
Summing all six cells: $5.83+8.05+0.55+0.76+2.79+3.86 = 21.84$, matching R's statistic above. For a test of independence, $df=(r-1)(c-1)$ for $r$ rows and $c$ columns: $(3-1)(2-1)=2$. The number of sites never enters. With $df=2$, the critical value at $\alpha=.05$ is `r round(qchisq(.95, 2), 2)`; our $21.84$ is more than three times that, which is where $p<.001$ comes from.

**Interpretation.** $\chi^2(2) = 21.84$, $p < .001$: significant. *E. coli* exceedance is not independent of land use. The test says the two variables are related; the table says how. Divide within each row to get the exceedance rate for each land use:

| Land use     | Exceeds / total | Share |
|:-------------|----------------:|------:|
| Forested     |          8 / 50 |   16% |
| Agricultural |         25 / 50 |   50% |
| Urban        |         30 / 50 |   60% |

Dividing within a row matters because raw counts only compare fairly when the row totals match. Agricultural and urban sites exceed the threshold three to four times as often as forested ones. Note what this test does *not* tell you: it says the two variables are related, but, just like correlation, it doesn't by itself establish that land use *causes* the exceedance.

::: {.callout-important title="Results reporting: nonparametric and chi-square tests"}
...
> Soil moisture did not change significantly after restoration, Wilcoxon signed-rank *V* = 39, *p* = .057 (median change +1.5 percentage points across all 12 plots; 9 non-zero differences).
