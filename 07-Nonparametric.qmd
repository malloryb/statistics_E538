...
ggplot(tibble(V = sim_V), aes(x = V)) +
  geom_histogram(binwidth = 2, color = "white", fill = "#008cff", boundary = 0) +
  ...
  geom_vline(xintercept = obs_V, color = "#10377f", linewidth = 1) +
  ...
  annotate("text", x = obs_V, y = Inf, label = paste0("observed V = ", obs_V),
           color = "#10377f", vjust = 5.5, hjust = 1.1, size = 3.3) +
  theme_classic(base_size = 13) +
...
ggplot(tibble(U = sim_U), aes(x = U)) +
  ...
  annotate("rect", xmin = -Inf, xmax = crit_lo, ymin = 0, ymax = Inf, alpha = 0.15, fill = "#b22222") +
  annotate("rect", xmin = crit_hi, xmax = Inf, ymin = 0, ymax = Inf, alpha = 0.15, fill = "#b22222") +
  geom_vline(xintercept = obs_U, color = "#10377f", linewidth = 1) +
  annotate("text", x = crit_lo, y = Inf, label = paste0("critical value: ", crit_lo),
           color = "#b22222", vjust = 3, hjust = 1.05, size = 3.3) +
  annotate("text", x = crit_hi, y = Inf, label = paste0("critical value: ", crit_hi),
           color = "#b22222", vjust = 3, hjust = -0.05, size = 3.3) +
  annotate("text", x = obs_U, y = Inf, label = paste0("observed U = ", obs_U),
           color = "#10377f", vjust = 1.5, hjust = 1.1, size = 3.3) +
  theme_classic(base_size = 13) +
...
How can a nonparametric test have its own named distribution? "Nonparametric" describes what you assume about the data: you don't assume they came from a particular family like the normal. Every test still needs a reference distribution for its test statistic. For the rank tests, that distribution is exact: it comes from every possible arrangement of the ranks under $H_0$. For the chi-square tests, the chi-square curve is an approximation to the statistic's distribution under $H_0$, and it is a good one only when expected counts are large enough. That's why each chi-square test below comes with an expected-count rule.
