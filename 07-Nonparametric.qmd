...
Where do the critical values actually come from? The same place they always do in this book: we build the null distribution ourselves, by simulating 20,000 experiments where each of the 9 nonzero ranks independently gets a random $+$ or $-$ sign, exactly what $H_0$ says should happen, and adding up the positive ranks each time.[This graph doesn't look right, it says critical value: 6 and it says critical value is 57, why both labels? And our prose says the critical value is 39?]

```{r}
#| label: fig-8wilcoxon-null
#| fig-cap: "Simulated null distribution of the Wilcoxon signed-rank statistic V, built from 20,000 random sign-flips of the 9 nonzero ranked differences. Shaded regions mark the two-tailed 5% rejection region; our observed V = 39 lands right on the boundary."
...
ggplot(tibble(V = sim_V), aes(x = V)) +
  ...
  annotate("text", x = obs_V, y = Inf, label = paste0("observed V = critical value = ", obs_V),
           color = "#1446a0", vjust = 1.5, hjust = 1.02, size = 3.3) +
  theme_classic(base_size = 13) +
...
**The full picture.** $t$-test: $p=.026$, significant. Wilcoxon signed-rank: $p=.057$, not quite. Sign test: $p=.508$, not close. Same 12 plots, three tests, three different verdicts [This soudns AI-y].
...
Now let's run through the same kind of example for the independent-samples case. Researchers surveyed **plant species richness**, the number of distinct plant species found, at restored and unrestored wetland plots, to see whether restoration increased plant diversity. @fig-8mwwdesign gives the design.

```{r}
#| label: fig-8mwwdesign
...
Same question as before: where do the critical values come from? We build them the same way, by simulation. Under $H_0$, group membership is random with respect to rank, so we pool the richness values, then repeatedly shuffle which 8 ranks land in the restored group:[Don't think the criti cal values were clearly created here, and "we build them teh same wya, by simulation" is not how they are generally built right, that's the simulation logic from before but we will eventually have a known distribution won't we? Or maybe not actually nevermind, but shoudl clarify that in the text]
...
Chapter 3 introduced the shape of the chi-square distribution: always non-negative, right-skewed, naturally one-tailed, with its exact shape depending on degrees of freedom. @fig-8chisq-crit shows what the rejection region looks like in practice, using the goodness-of-fit example we'll run below, which has $df = 3$. The critical value at $\alpha = .05$ is `r round(qchisq(.95, 3), 2)`, and any statistic beyond that point (the shaded region) occurs less than 5% of the time by chance.
...
A state wildlife agency has historically found fish evenly distributed across four zones of a lake, 25% in each. This year's survey of 200 fish gives a different picture. @fig-8gofdesign lays out the structure, and note how it differs from every design so far in this book.

```{r}
#| label: fig-8gofdesign
...
Now suppose researchers classify 150 stream sites by **land use** and by whether each site's *E. coli* level **exceeds** a recreational safety threshold. Every site falls into exactly one cell of @fig-8indepdesign.

```{r}
#| label: fig-8indepdesign
