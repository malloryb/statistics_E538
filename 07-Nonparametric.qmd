...
A state wildlife agency has historically found fish evenly distributed across four zones of a lake, 25% in each. This year's survey of 200 fish gives a different picture. @fig-7gofdesign lays out the structure, which differs from every design so far in this book.
...
In that table, nobody assigned anything. The sites already had their land use and their *E. coli* level before the researchers arrived, and the cells simply count how many turned out each way. That makes this an observational design, and it is why the cells hold counts of sites rather than measurements taken on them.
...
All six expected counts are between 21 and 29, well above the 5-count minimum. This check does not tell us whether the test is significant. It only tells us whether the chi-square approximation is trustworthy here.
...
**Interpretation.** $\chi^2(2) = 21.84$, $p < .001$: significant. *E. coli* exceedance is not independent of land use. To see how the two variables are related, look at the table. Divide within each row to get the exceedance rate for each land use:
...
Dividing within a row matters because raw counts only compare fairly when the row totals match. Agricultural and urban sites exceed the threshold three to four times as often as forested ones. The test says the two variables are related, but, like correlation, it does not by itself establish that land use *causes* the exceedance.

::: {.callout-important title="Results reporting: nonparametric and chi-square tests"}
These tests report differently from a $t$-test. There is no mean difference to quote, because none of them used the means. Give the test's own statistic, the $p$-value, and a **median** rather than a mean.
...
Report the number of non-zero differences for a signed-rank test, since dropped zeros change the effective sample size. And for chi-square, give the actual counts, not just the statistic: the counts are what a reader can interpret.
:::
