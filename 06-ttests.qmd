...
Chapter 5 introduced the concept: a two-tailed test asks whether $\mu$ differs from $\mu_0$ in either direction, while a one-tailed test commits to one direction in advance, and is only justified when your theory already rules out the other direction. Here's how that choice plays out mechanically for a $t$-test: it changes the critical value, and therefore the $p$-value, for the exact same data.
