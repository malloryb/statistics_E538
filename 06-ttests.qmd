...
With the single $t$ from our sample, we can check @fig-6-t-rejection to see whether a $t$ that size is likely or unlikely by chance alone.
...
Older textbooks listed these values in a table at the back of the book. R, Excel, and [online calculators](http://www.socscistatistics.com/pvalues/tdistribution.aspx) all work; you'll get hands-on practice with this in lab, both with software and by simulation.
...
What happens if there is a real difference in the simulated data? Let's set one condition to have a larger mean than the other:
...
We got `r length(save_ps[save_ps<.05])` simulations where p was less than .05, or `r 100*length(save_ps[save_ps<.05])/1000`% of experiments. A design that rarely detects a real effect needs to be improved before you run it.
