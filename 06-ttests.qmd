...
3.  **Independent-samples t-test (also called two-sample t-test)** Compare means from two groups measured on different units. The pooled version assumes the two groups have similar spread; Welch's version [@Welch1947] does not, and it is what R's `t.test()` runs unless you set `var.equal = TRUE`.
...
Every $t$ formula so far has carried $df = n-1$ alongside it, and @fig-6-z-vs-t-linear-log drew a different $t$ curve for each $df$. That number is the degrees of freedom, and it is worth understanding what it means, not just where to find it. It is also the number in parentheses whenever a $t$-test is reported, as in $t(9) = 1.94$.
...
We can see the effect of degrees of freedom directly by simulating the $t$ distribution at two different sample sizes. @fig-6dft compares $df=9$ (a sample of 10) to $df=100$ (a much larger sample).
...
ggplot(t_df, aes(x = ts)) +
  geom_histogram(fill = "#008cff", color = "white") +
  facet_wrap( ~ dfs) +
  xlab("t value") +
  ylab("Count")
...
**A useful anchor:** at $\alpha=.05$ two-tailed and $df\approx 20$, $|t|\gtrsim 2$ is already uncommon under $H_0$; smaller $df$ pushes that threshold higher, as the critical values above show.
...
**Test-specific assumptions.** These come from how a particular test's math actually works, and they differ test to test. For the one-sample and paired $t$-tests, the specific assumption is **approximate normality** of the outcome itself (one-sample) or of the difference scores (paired). Why does this matter? The $t$-distribution we've been using to get $p$-values is *derived* assuming the sampling distribution of the sample mean is normal. If your data are badly skewed and your sample is small, that derivation is on shaky ground, and the $p$-value it hands you may not mean what it claims to mean.

**How worried should you be?** It depends enormously on $n$. With a large sample, the Central Limit Theorem does a lot of the work for you. The sampling distribution of the sample mean tends toward normal even when individual observations are skewed, so the $t$-test is fairly **robust** to non-normality once $n$ is reasonably large. A lot of environmental data, particularly field data, instead comes from **small samples that are also skewed**. Examples include a handful of expensive water samples, a short list of monitoring wells, five plots you had time to visit before the storm rolled in. Small $n$ and non-normality reinforcing each other is when a $t$-test's answer becomes questionable.
...
#| fig-cap: 'Two-tailed rejection regions at $\alpha=.05$ for $t_{(df=9)}$. The shaded tails beyond the red critical values mark unlikely values under $H_0$.'
#| fig-alt: "A blue t-distribution curve with 9 degrees of freedom. Red vertical lines labeled minus crit and plus crit mark the critical values near plus and minus 2.26, and the two tails beyond them are shaded red and labeled 'rejection region.' The middle of the curve is labeled 'retain H0.'"
#| out-width: "80%"

library(tidyverse)

df   <- 9
xs   <- seq(-4.5, 4.5, by = 0.005)
t_tibble <- tibble(x = xs, y = dt(xs, df))
crit <- qt(.975, df)

ggplot(t_tibble, aes(x, y)) +
  geom_area(data = subset(t_tibble, x <= -crit), fill = "red", alpha = 0.18) +
  geom_area(data = subset(t_tibble, x >=  crit), fill = "red", alpha = 0.18) +
  ...
  annotate("text", x = c(-crit, crit), y = 0.425, label = c("-crit", "+crit"),
           colour = "red", size = 4.2) +
  annotate("text", x = -3.4, y = 0.06, label = "rejection\nregion", size = 3.4, color = "grey20") +
  annotate("text", x = 3.4, y = 0.06, label = "rejection\nregion", size = 3.4, color = "grey20") +
  annotate("text", x = 0, y = 0.2, label = "retain~H[0]", parse = TRUE, size = 3.8, color = "grey20") +
  annotate("text", x = 2.45, y = 0.3, label = "'Under '*H[0]", parse = TRUE, colour = "blue", hjust = 0, size = 4.2) +
  labs(x = "t", y = "Density")
```

### Interpreting $t$

We're asking whether our $t$-value could plausibly have come from the null distribution: the distribution of $t$-values we'd see across many samples of size 10, if the true mean were exactly 50 (chance level). @fig-6tnull shows what that distribution looks like, at $df=9$.
...
#| fig-alt: "A symmetric t-distribution curve with 9 degrees of freedom, centered at zero with a grey vertical line marking the center. Labels on the left and right halves each read 50 percent, showing the curve splits evenly into negative and positive t-values."
...
ggplot(plot_df, aes(x = range, y = null_distribution)) +
  geom_line(colour = "blue", linetype = "dotdash", linewidth = 1.1) +
  annotate("segment", x = 0, xend = 0, y = 0, yend = dt(0, 9), colour = "grey40", linewidth = 0.8) +
  annotate("text", x = c(-.7, .7), y = .1, label = c("50%\n(-)", "50%\n(+)"), size = 4.5, colour = "grey20") +
...
#| fig-alt: "The same t-distribution curve with 9 degrees of freedom, divided by nine grey vertical lines into ten regions, each labeled 10 percent. The regions near the two outer edges are noticeably wider than the regions clustered near the center of the curve."
#| out-width: "100%"
...
# Decile boundaries are guides, not critical values, so grey and stopping at the curve.
ggplot(plot_df, aes(x = range, y = null_distribution)) +
  geom_line(colour = "blue", linetype = "dotdash", linewidth = 1.1) +
  annotate("segment", x = t_ps, xend = t_ps, y = 0, yend = dt(t_ps, 9), colour = "grey40", linewidth = 0.7) +
  scale_x_continuous(breaks = seq(-3, 3, 1)) +
  # one label per region, turned sideways so the narrow central regions fit
  annotate("text", x = c(-1.75, (t_ps[-9] + t_ps[-1]) / 2, 1.75), y = 0.04, angle = 90,
           label = "10%", size = 3.6, colour = "grey20") +
  labs(x = "t", y = "Density")
...
#| fig-alt: "A t-distribution curve with 9 degrees of freedom, with a single red vertical line at t equal to 1.83 labeled plus crit equals 1.83. The tail to the right of that line is shaded red and labeled 5 percent; everything to the left is unshaded. A green dotted line marks the observed t of 1.94, just inside the shaded tail."

range <- seq(-3.5, 3.5, .01)
plot_df <- data.frame(range, null_distribution = dt(range, 9))
crit1 <- qt(.95, 9)
t_obs <- 1.94   # the quiz example, t(9) = 1.94

ggplot(plot_df, aes(x = range, y = null_distribution)) +
  geom_area(data = subset(plot_df, range >= crit1), fill = "red", alpha = 0.18) +
  geom_line(colour = "blue", linetype = "dotdash", linewidth = 1.1) +
  annotate("segment", x = crit1, xend = crit1, y = 0, yend = 0.41, colour = "red", linewidth = 0.8) +
  annotate("text", x = crit1, y = 0.425, label = paste0("+crit = ", round(crit1, 2)), colour = "red", size = 4.2) +
  annotate("segment", x = t_obs, xend = t_obs, y = 0, yend = 0.33, colour = "#1E9E1E", linetype = "dotted", linewidth = 1.4) +
  annotate("text", x = t_obs + 0.08, y = 0.36, label = "observed\nt = 1.94", colour = "#1E9E1E", hjust = 0, size = 3.8, lineheight = 0.9) +
  annotate("text", x = 2.9, y = 0.06, label = "5%", size = 4.2, colour = "grey20") +
  labs(x = "t", y = "Density")
```
...
#| fig-alt: "A t-distribution curve with 9 degrees of freedom, with two red vertical lines labeled minus crit and plus crit at negative 2.26 and positive 2.26. The tails beyond each line are shaded red and labeled 2.5 percent, one on each side, leaving the large central region between them unshaded. A green dotted line marks the observed t of 1.94, just short of the right critical value."
...
ggplot(plot_df, aes(x = range, y = null_distribution)) +
  geom_area(data = subset(plot_df, range <= -crit2), fill = "red", alpha = 0.18) +
  geom_area(data = subset(plot_df, range >=  crit2), fill = "red", alpha = 0.18) +
  geom_line(colour = "blue", linetype = "dotdash", linewidth = 1.1) +
  annotate("segment", x = c(-crit2, crit2), xend = c(-crit2, crit2), y = 0, yend = 0.41, colour = "red", linewidth = 0.8) +
  annotate("text", x = c(-crit2, crit2), y = 0.425,
           label = paste0(c("-crit = -", "+crit = "), round(crit2, 2)), colour = "red", size = 4.2) +
  annotate("segment", x = t_obs, xend = t_obs, y = 0, yend = 0.33, colour = "#1E9E1E", linetype = "dotted", linewidth = 1.4) +
  annotate("text", x = t_obs - 0.08, y = 0.36, label = "observed\nt = 1.94", colour = "#1E9E1E", hjust = 1, size = 3.8, lineheight = 0.9) +
  annotate("text", x = c(-3.4, 3.4), y = 0.06, label = "2.5%", size = 4.2, colour = "grey20") +
  labs(x = "t", y = "Density")
```
...
The rule is Chapter 4's, unchanged: default to two-tailed, and go one-tailed only when a result in the opposite direction would lead you to do the same thing as no result. What the $t$-test adds is that you can now see the cost directly in the critical value, since $t_{crit}$ for a one-tailed test sits closer to 0 than the two-tailed one at the same $\alpha$.
...
#| fig-alt: "Study design schematic. IV: restoration, before versus after. Two dashed boxes labeled Before and After, each n = 20 plots, with a note that these are the same 20 plots measured on both occasions. DV: soil moisture, percent."
#| fig-asp: 0.34
#| out-width: "100%"
...
1.  Decide what the population looks like. Here each sample is 30 sites, with values drawn from a normal distribution with mean 50 and SD 25.
2.  Draw one sample from that population, run the $t$-test, and save the statistics you care about ($t$ and $p$). Repeat 1,000 times and look at how they are distributed.

Every test compares the sample mean to $\mu_0 = 50$, which is also the true population mean, so $H_0$ is true in every repetition. @fig-6nullt is the null distribution of $t$ that this produces.
...
#| fig-alt: "Histogram of 1000 simulated p-values from samples of size 10 with a real underlying difference. Unlike the flat null histogram, this one is right-skewed, with the tallest bars near zero and progressively shorter bars moving toward 1."
hist_p(save_ps)
```

Now you can see that the $p$-value distribution is right-skewed: most of the values pile up near 0, with a long tail toward 1. This is because when there is a true effect, you will get p-values that are less than .05 more often. Or, rather, you get larger $t$ values than you normally would if there were no differences.
...
Now almost all of the experiments (`r sum(save_ps < .05)` of 1000) show a $p$-value of less than .05 (using a two-tailed test, the default in R). You can use this simulation process to determine how many subjects you need to reliably detect your effect.
