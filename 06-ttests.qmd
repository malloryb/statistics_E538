...
Every $t$ formula so far has carried $df = n-1$ alongside it, and @fig-6-z-vs-t-linear-log drew a different $t$ curve for each $df$. That number is the degrees of freedom. It is also the number in parentheses whenever a $t$-test is reported, as in $t(9) = 1.94$.
...
At $n=20$, $t(19) = `r round(full20_out$statistic, 2)`$, $p =`r fmt_p(full20_out$p.value)`$: significant. The mean difference (`r round(full20_out$estimate, 1)` percentage points) is in the same range as the 5-plot subset suggested. The standard error shrank enough with the larger sample to rule out chance, so the extra plots gave more power to detect a real effect.
