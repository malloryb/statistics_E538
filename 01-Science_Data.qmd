...
### Why we need Statistics: Simpson's paradox
...
Here's how that could look, and we'll use case fatality rate (CFR). This is a real phenomenon but I made up numbers for this. 

```{r}
#| label: fig-simpson-covid-overall
#| fig-cap: "CCrude case fatality rates. Italy appears to have a higher fatality rate than China when all cases are combined."
library(ggplot2)
library(tibble)
...
At face value, Italy looks worse. The crude rate is higher in Italy, which could tempt a quick conclusion if you stop there. But what happens when we disaggregate by age group? We know age is a major confounder (define) for COVID fatality, so let's stratify by 10-year bands. When we do, China’s fatalitiy rate is higher in every band.
...
ggplot(fig3_df, aes(x = Age, y = Prop, fill = Country, group=Country)) +
  ...
  theme_classic(base_size = 13) +
  theme(legend.position = "top", panel.border = element_rect(size = 1, fill=NA))
```
