...
At face value, Italy looks worse. The crude rate is higher in Italy, which could tempt a quick conclusion if you stop there. But what happens when we disaggregate by age group? Age is a confounder, so you stratify by 10-year bands. Look, point out that within every band China’s CFR is higher.

```{r}
#| label: fig-simpson-by-age-group
#| fig-cap: "Fatality rates by age group (same made-up numbers)."
...
Italy’s cases skew older while China’s skew younger, which flips the crude comparison. Then give the moral: check stratified results and adjust for key covariates when you can. Within every age group, China’s fatality rate is higher. The reversal arises because China had many more young cases, and younger groups have much lower fatality rates. Aggregating the data without age adjustment hid this reality.

```{r}
#| label: fig-age-distributions
#| fig-cap: "Age distribution by country.  Italy is older, China is younger."
