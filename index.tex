...
\begin{tcolorbox}[enhanced jigsaw, breakable, colback=white, opacitybacktitle=0.6, coltitle=black, toprule=.15mm, bottomtitle=1mm, leftrule=.75mm, arc=.35mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Takeaway}, opacityback=0, colframe=quarto-callout-note-color-frame, left=2mm, titlerule=0mm, toptitle=1mm, colbacktitle=quarto-callout-note-color!10!white, rightrule=.15mm, bottomrule=.15mm]
...
\begin{tcolorbox}[enhanced jigsaw, breakable, colback=white, opacitybacktitle=0.6, coltitle=black, toprule=.15mm, bottomtitle=1mm, leftrule=.75mm, arc=.35mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, opacityback=0, colframe=quarto-callout-note-color-frame, left=2mm, titlerule=0mm, toptitle=1mm, colbacktitle=quarto-callout-note-color!10!white, rightrule=.15mm, bottomrule=.15mm]
...
Figure~\ref{fig-4samplingmean} animates this idea. Each panel shows
...
\includegraphics[width=0.75\linewidth,height=\textheight,keepaspectratio]{imgs/gifs/sampleDistNormal-1.gif}

}

\caption{\label{fig-4samplingmean}Simulation of samples (grey), their
means (red line), and the sampling distribution of the mean (blue). As
sample size grows, the distribution of sample means narrows and centers
...
\begin{tcolorbox}[enhanced jigsaw, breakable, colback=white, opacitybacktitle=0.6, coltitle=black, toprule=.15mm, bottomtitle=1mm, leftrule=.75mm, arc=.35mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Tip}, opacityback=0, colframe=quarto-callout-tip-color-frame, left=2mm, titlerule=0mm, toptitle=1mm, colbacktitle=quarto-callout-tip-color!10!white, rightrule=.15mm, bottomrule=.15mm]
...
\begin{tcolorbox}[enhanced jigsaw, breakable, colback=white, opacitybacktitle=0.6, coltitle=black, toprule=.15mm, bottomtitle=1mm, leftrule=.75mm, arc=.35mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Pro tip}, opacityback=0, colframe=quarto-callout-tip-color-frame, left=2mm, titlerule=0mm, toptitle=1mm, colbacktitle=quarto-callout-tip-color!10!white, rightrule=.15mm, bottomrule=.15mm]
...
\(p\)-value. Assuming \(H_0\) is true, the \(p\)-value is the
...
\item
  \(p\)-value (probability). Compute the \(p\)-value for your observed
  statistic under \(H_0\) and compare to \(\alpha\). Reject \(H_0\) if
  \(p<\alpha\).
\end{enumerate}
...
true effects are harder to detect, and \(\beta\) increases (power
decreases). As \(n\) grows, the sampling distribution tightens,
\(\beta\) falls, and power rises. Apparent changes in the Type I rate at
small \(n\) usually reflect \textbf{assumption violations} (e.g.,
non-normality, poor variance estimates, multiple looks), not \(n\)
itself.
...
In the early 1900s, William Sealy Gosset joined \textbf{Guinness}, then
the world's largest brewery. Guinness hired scientifically trained
brewers and gave them latitude to pursue research that improved the
product. As production scaled, quality control mattered: a pint should
taste the same in Dublin as in Detroit. Until then, hop quality was
judged mostly by appearance and smell, which was unreliable. Chemical
assays could measure ingredients, but the core problem was statistical.
Inference relied on the normal (Z) distribution, which works well when
samples are large enough for a normal approximation. In practice,
Guinness could not destroy large volumes of beer or ingredients just to
test them.

Gosset set out to quantify the extra uncertainty introduced by
\textbf{small} samples. How much wider is the error around an estimate
when \(n=10\) instead of \(n=1000\)? He derived a new sampling
distribution that accounts for small-sample variability, now known as
the \textbf{Student's (t) distribution}. Because Guinness restricted
external publications, he published the 1908 paper under the pseudonym
\textbf{``Student''} (Student 1908). That is why we still say
\textbf{Student's \(t\)-test}. The core idea you will use today is the
same: measure a signal, scale it by its standard error, and compare the
resulting \(t\) to the appropriate \(t\) distribution for your \(n-1\)
degrees of freedom.

\textbf{Why this matters.} We often compare a sample mean to a reference
(drinking-water standard, historical average) or compare two conditions
(before/after, site A vs site B). A t-test asks whether the observed
difference is larger than you would expect from sampling variation
alone, given your \(n\) and variability.

\textbf{Three common t-tests you will use.}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{One-sample t-test.} Compare a sample mean to a target
  (\(\mu_0\)).
\item
  \textbf{Paired t-test.} Compare before/after (or matched) measurements
  on the same units by running a one-sample t on the \emph{differences}.
\item
  \textbf{Independent-samples t-test (also called two-sample t-test)}
  Compare means from two groups measured on different units. Use Welch's
  test by default when variances differ.
\end{enumerate}

\textbf{What you will report.}

Always report: * the effect size (mean difference) * a 95\% CI, the test
statistic with df * and the \(p\)-value.

Example: ``Mean change was (-0.42) units (95\% CI {[}-0.73,-0.11{]}
;t(11)=-2.92; p=0.014).''

\section{From mean \& SD to ``signal over
noise''}\label{from-mean-sd-to-signal-over-noise}

A mean tells you where the center is; the SD tells you how spread out
the data are. A mean of 50 could be very representative (if SD is tiny)
or nearly meaningless (if SD is huge). You need both.
...
\begin{itemize}
\item
  Big numerator, small denominator → big ratio (more confidence that the
  mean reflects the data).
\item
  Small numerator or big denominator → small ratio (less confidence).
\end{itemize}

For instance, mean = 50 with SD = 1 gives 50; mean = 50 with SD = 100
gives 0.5. Same mean, very different stories.

This ``signal over noise'' pattern shows up everywhere in inference. In
general,
...
For \(t\)-tests, we won't actually use SD in the denominator, we use the
\textbf{standard error} \(SE\), which is the SD of the sample mean. But
the logic is the same: measure a signal, scale it by its uncertainty,
and see whether the resulting ratio is unusually large or small under
the right \(t\) distribution.

\[
t = \frac{\text{effect}}{\text{standard error}} = \frac{\bar{X} - \mu_0}{s / \sqrt{n}}
\]

We now turn that ratio into a test statistic for a \emph{single mean}.

\section{One-sample t-test}\label{one-sample-t-test}

Use a one-sample \(t\) when you want to test whether a sample mean
differs from a reference value (\(\mu_0\)). The question: is the
observed difference bigger than you'd expect from sampling variability,
given \(n\) and your sample SD \(s\)?

\[
t = \frac{\bar{X}-\mu_0}{s/\sqrt{n}}, \qquad df = n-1
\]

After calculating \(t\) from our data, we compare the observed \(t\) to
a \(t\) distribution with \(n-1\) degrees of freedom to get a
\(p\)-value and a decision at \(\alpha\).

\subsection{\texorpdfstring{Comparing \(z\) and
\(t\)}{Comparing z and t}}\label{comparing-z-and-t}

When the population SD is known (or we have a large enough sample to
assume convergence), the ratio is called \(z\); when it's estimated from
the data, it's \(t\).''

Under a known population SD (\(\sigma\)), the standardized mean uses the
\textbf{\(z\)} statistic: \[
z = \frac{\bar{X}-\mu_0}{\sigma/\sqrt{n}}
\]

When \(\sigma\) is \textbf{unknown} (the usual case), we estimate it
with the sample SD (\(s\)) and use the \textbf{\(t\)} statistic: \[
t = \frac{\bar{X}-\mu_0}{s/\sqrt{n}}, \qquad df = n-1
\]

Because \(s\) varies from sample to sample, the \(t\) distribution has
\textbf{heavier tails} than the standard normal. As \(n\) grows, \(s\)
stabilizes and \(t\) \textbf{converges} to \(z\).

\begin{figure}

\centering{

\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{07-ttests_files/figure-pdf/fig-7-z-vs-t-linear-log-1.pdf}

}

\caption{\label{fig-7-z-vs-t-linear-log}Normal (\(z\)) vs \(t\)
distributions: on a linear scale (top), tail differences look small; on
a log scale (bottom), the heavy tails of small-\(n\) \(t\) are clear.}

\end{figure}%

\begin{tcolorbox}[enhanced jigsaw, breakable, colback=white, opacitybacktitle=0.6, coltitle=black, toprule=.15mm, bottomtitle=1mm, leftrule=.75mm, arc=.35mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, opacityback=0, colframe=quarto-callout-note-color-frame, left=2mm, titlerule=0mm, toptitle=1mm, colbacktitle=quarto-callout-note-color!10!white, rightrule=.15mm, bottomrule=.15mm]

\textbf{When is \(z\) appropriate?} If the population SD \(\sigma\) is
known \emph{and} the sampling distribution of the mean is
(approximately) normal, use \(z\). In practice, \(\sigma\) is rarely
known, so we use \(t\) with \(s\) and \(df=n-1\).

\end{tcolorbox}

\textbf{Rule of thumb.} For two-tailed tests at \(\alpha=0.05\):
...
\item
  \(t\) is a bit wider for small \(n\) (e.g.,
  \(df=9 \Rightarrow \pm 2.262\); \(df=29 \Rightarrow \pm 2.045\)), and
  approaches \(\pm 1.96\) as \(n\) increases.
\end{itemize}

\subsection{What t represents}\label{what-t-represents}

Either way, the ratio asks the same question: how far is the mean from
the reference in SE units?

\(t\) expresses how far a sample mean is from the hypothesized mean, in
\textbf{standard error units} (signal over noise):

\[
t = \frac{\bar{X}-\mu_0}{s/\sqrt{n}}
\]

\(t\) can be positive or negative depending on direction. Under \(H_0\),
\(t \sim t_{(df=n-1)}\).\\
\textbf{Rule of thumb:} at \(\alpha=.05\) two-tailed, \(|t|\gtrsim 2\)
is uncommon when \(df\approx 20\).

\begin{tcolorbox}[enhanced jigsaw, breakable, colback=white, opacitybacktitle=0.6, coltitle=black, toprule=.15mm, bottomtitle=1mm, leftrule=.75mm, arc=.35mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, opacityback=0, colframe=quarto-callout-note-color-frame, left=2mm, titlerule=0mm, toptitle=1mm, colbacktitle=quarto-callout-note-color!10!white, rightrule=.15mm, bottomrule=.15mm]

Remember, \(s/\sqrt{n}\) is the \textbf{standard error (SE)}---the
standard deviation of the sampling distribution of the mean.

\end{tcolorbox}

Let's see how \(t\) values behave when samples really come from the null
hypothesis. Under the null hypothesis, \(t\) follows a \(t\)
distribution with \(df = n-1\).\\
Large \(|t|\) values are rare; with \(df \approx 20\), \(|t| \gtrsim 2\)
is uncommon at \(\alpha = .05\) (two-tailed).

\begin{figure}

\centering{

\includegraphics[width=0.75\linewidth,height=\textheight,keepaspectratio]{07-ttests_files/figure-pdf/fig-7t-sim-1.pdf}

}

\caption{\label{fig-7t-sim}Histogram of 1,000 simulated \(t\) values
(\(n=10\)) drawn from a population with \(\mu_0=0\). Most \(t\)s cluster
near 0; large positive or negative values are rare.}

\end{figure}%

\textbf{Takeaway:} \(t\) measures how far the sample mean is from the
null expectation, scaled by its uncertainty. It behaves predictably
under \(H_0\), allowing us to decide whether a difference is likely due
to chance.

\subsection{Calculating t from data}\label{calculating-t-from-data}

Let's calculate a one-sample \(t\) statistic step by step using a short
example. Ten students each took a 5-question true/false quiz (50\%
expected by chance). Their scores (percent correct) are:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{scores }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\DecValTok{50}\NormalTok{, }\DecValTok{70}\NormalTok{, }\DecValTok{60}\NormalTok{, }\DecValTok{40}\NormalTok{, }\DecValTok{80}\NormalTok{, }\DecValTok{30}\NormalTok{, }\DecValTok{90}\NormalTok{, }\DecValTok{60}\NormalTok{, }\DecValTok{70}\NormalTok{, }\DecValTok{60}\NormalTok{)}
\NormalTok{n  }\OtherTok{\textless{}{-}} \FunctionTok{length}\NormalTok{(scores)}
\NormalTok{x  }\OtherTok{\textless{}{-}} \FunctionTok{mean}\NormalTok{(scores)}
\NormalTok{s  }\OtherTok{\textless{}{-}} \FunctionTok{sd}\NormalTok{(scores)}
\NormalTok{SE }\OtherTok{\textless{}{-}}\NormalTok{ s }\SpecialCharTok{/} \FunctionTok{sqrt}\NormalTok{(n)}

\NormalTok{x; s; SE}
\CommentTok{\#\textgreater{} [1] 61}
\CommentTok{\#\textgreater{} [1] 17.91957}
\CommentTok{\#\textgreater{} [1] 5.666667}

\NormalTok{tt }\OtherTok{\textless{}{-}} \FunctionTok{t.test}\NormalTok{(scores, }\AttributeTok{mu =} \DecValTok{50}\NormalTok{)  }\CommentTok{\# Welch one{-}sample t}

\NormalTok{tt}\SpecialCharTok{$}\NormalTok{statistic      }\CommentTok{\# t}
\CommentTok{\#\textgreater{}        t }
\CommentTok{\#\textgreater{} 1.941176}
\NormalTok{tt}\SpecialCharTok{$}\NormalTok{parameter      }\CommentTok{\# df}
\CommentTok{\#\textgreater{} df }
\CommentTok{\#\textgreater{}  9}
\NormalTok{tt}\SpecialCharTok{$}\NormalTok{p.value        }\CommentTok{\# p}
\CommentTok{\#\textgreater{} [1] 0.08415031}
\NormalTok{tt}\SpecialCharTok{$}\NormalTok{conf.int       }\CommentTok{\# 95\% CI}
\CommentTok{\#\textgreater{} [1] 48.18111 73.81889}
\CommentTok{\#\textgreater{} attr(,"conf.level")}
\CommentTok{\#\textgreater{} [1] 0.95}
\NormalTok{tt}\SpecialCharTok{$}\NormalTok{estimate       }\CommentTok{\# sample mean}
\CommentTok{\#\textgreater{} mean of x }
\CommentTok{\#\textgreater{}        61}
\end{Highlighting}
\end{Shaded}

\textbf{Results (R):} \(t(9)=1.94\), \(p=0.0842\); mean \(=61\) (95\% CI
(48.18, 73.82)).

\textbf{Interpretation:} The mean (61\%) is above chance (50\%), but not
statistically significant at \(\alpha=.05\).

\textbf{Assumptions check:} Independence of observations; approximate
normality of the outcome (with \(n=10\), mild deviations are acceptable
for a one-sample \(t\)).

We can confirm this with a single R command:

\texttt{t.test(scores,\ mu\ =\ 5}
...
\textbf{Interpretation:} The sample mean (61\%) is higher than chance
(50\%), but the difference is not large enough to be statistically
significant at \(\alpha = 0.05\).

\textbf{Assumptions check:} Observations are independent, and with
\(n=10\) the data are approximately normal---sufficient for a one-sample
\(t\) test.

\subsection{How does t behave?}\label{how-does-t-behave}

Under the null hypothesis (\(H_0\) true), \(t\) follows a \emph{t}
distribution with degrees of freedom \(df = n - 1\). Large \(|t|\)
values are rare---extreme sample means are unlikely if \(H_0\) is true.

For moderate sample sizes (\(df \approx 20\)), a simple rule of thumb
is:
...
This means values of \(t\) beyond roughly \(-2\) or \(+2\) occur less
than 5\% of the time under the null. That's the foundation of the
decision rule for \(t\) tests: if your observed \(t\) lies in those rare
tails, you reject \(H_0\).

Remember, if we obtained a single \(t\) from one sample we collected, we
could consult the chance window in Figure~\ref{fig-7-t-rejection} below
to find out whether the \(t\) we obtained from the sample was likely or
...
\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{07-ttests_files/figure-pdf/fig-7-t-rejection-1.pdf}

}

\caption{\label{fig-7-t-rejection}Two-tailed rejection regions at
\(\alpha=.05\) for \(t_{(df=9)}\). Shaded tails mark unlikely values
under \(H_0\).}

\end{figure}%

\section{Paired t: before/after or
matched}\label{paired-t-beforeafter-or-matched}

When the \textbf{same units} are measured twice (before/after, matched
pairs), the question is whether they \textbf{changed}. The natural data
unit is the \textbf{within-unit difference}.

\textbf{Design in this week's lab (Mehr, Song, \& Spelke, 2016).}
Infants viewed two unfamiliar singers. At \textbf{baseline}, infants saw
silent videos and researchers recorded the \textbf{proportion of looking
time to the singer who would later sing the familiar song}. At
\textbf{test}, after each singer performed, infants again watched silent
videos and proportion looking was recorded. We care whether infants
\textbf{shifted attention} toward the familiar-song singer from baseline
to test.

\textbf{Define differences and their meaning.} Choose an order and stick
to it:

\begin{itemize}
\tightlist
\item
  (d\_i = \text{Test}\_i - \text{Baseline}\_i)
\item
  (d\_i\textgreater0): infant looked \textbf{more} at the familiar-song
  singer at test (increase)
\item
  (d\_i\textless0): infant looked \textbf{less} at the familiar-song
  singer at test (decrease)
\end{itemize}

This subtraction \textbf{cancels between-infant variability} (some
babies look around more than others) and focuses on
\textbf{within-infant change}.

\textbf{Test on differences (paired (t)).} Compute one difference per
infant, then run a \textbf{one-sample (t)} against 0:
...
The \textbf{sign of (t)} matches (\bar d) and your difference direction.

\textbf{Mini walk-through (first 5 infants).} Using the first five rows
you'll see in lab, suppose (\bar d = 0.054) and (s\_d = 0.168). Then \[
t=\frac{0.054}{0.168/\sqrt{5}} \approx 0.72,\qquad df=4,
\] which (two-tailed) is \textbf{not} significant. That's expected with
(n=5).

\textbf{Full sample effect.} With all infants (e.g., (n=32)), (\bar d)
is similar but (s\_d/\sqrt{n}) gets smaller, so (t) increases (in the
paper's data, it's significant). This is the payoff of pairing: more
power from the same number of participants.

\textbf{What to report.} (\bar d) (the mean change), a 95\% CI, (t(df)),
and (p). Optionally add a standardized effect (Cohen's (d=\bar d/s\_d)).

\textbf{R pointers (you'll do this in lab).}

\begin{itemize}
\tightlist
\item
  Compute differences: \texttt{d\ \textless{}-\ Test\ -\ Baseline}
\item
  Paired (t): \texttt{t.test(d,\ mu\ =\ 0)} or
  \texttt{t.test(Test,\ Baseline,\ paired\ =\ TRUE)}
\item
  CI for (\bar d): from the \texttt{t.test} output
\end{itemize}
...
\section{Two independent groups}\label{two-independent-groups}
...
\CommentTok{\#\textgreater{} [1] 0.063}
\end{Highlighting}
\end{Shaded}

We happened to make 63. The expectation over the long run is 5\% type I
...
means that for each level of one IV, the levels of the other IV are also
included.

\section{Factorial basics}\label{factorial-basics}

\textbf{Why This Matters}: Factorial designs let us study how multiple
environmental drivers act together. This is often a more realistic
picture of environmental systems.
...
\item
  ...
  pollution on macroinvertebrate diversity.
\end{enumerate}
...
The macroinvertebrate community clearly takes a hit from pollution, with
noticeably lower diversity in the industrial waste condition. In
general, it's common to use the word \textbf{effect} to describe the
difference caused by a manipulation. In this case, we could call it the
``Pollution effect.''
...
Figure~\ref{fig-10flowB} plots the same data in a way that makes it
easier to see what we are talking about.
...
Before using full ANOVA syntax, we'll use \(t\)-tests to illustrate the
same ideas
...
Is a difference of this size likely or unlikely due to chance? We could
...
Pollution and Flow was not significant, \(t\)(4) = 2.493, \(p\) = 0.067.
...
The two-way interaction between Pollution and Flow was not significant,
t(4) = 2.493, p = 0.067. The mean Pollution effect in the no-Flow
condition was 6 and the mean Pollution effect in the Flow condition was
2.6.
...
The two-way interaction between Pollution and Flow was not significant,
\(t\)(4) = 2.493, \(p\) = 0.067. The mean Pollution effect in the
...
Sometimes it helps to pause and reflect on where we are. We've covered a
lot of ground in this chapter---some of the most complex material in the
course so far.

We could keep going and work through more ANOVAs by hand. That practice
can be valuable once or twice because it shows exactly how the sums of
squares are calculated and helps you understand what the ANOVA is doing
under the hood. But beyond that, it mostly becomes bookkeeping rather
than learning. It's enough to know that you could do it, and to
recognize what each part of the output represents.

From here, we'll work at a slightly higher level. We'll focus on how to
design studies, choose the right type of ANOVA, and interpret the
results that R produces. This is what you'll do in the lab, and what
researchers and analysts do in practice. Software does the arithmetic,
but you still need to know what it's doing so you can check that the
results make sense. That judgment comes with experience, repetition, and
a clear understanding of the logic behind the analysis.

So, for the rest of our discussion of ANOVAs, we'll let R handle the
calculations and keep our focus on what the results mean. No more giant
tables of sums of squares---just thoughtful interpretation.
...
The answer is that we reject the null, and accept the alternative 175
times out of 1000. In other words our experiment succesfully accepts the
alternative hypothesis 17.5 percent of the time, this is known as the
...
Now only 144 out of 1000 experiments are significant, that's 14.4 power.
