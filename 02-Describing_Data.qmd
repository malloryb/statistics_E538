...
The mean is unique: it is the only single value that can replace every observation so that the total stays the same. This balancing property is why the mean plays such a central role in statistics and why we’ll return to it in later chapters.
:::
