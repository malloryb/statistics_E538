...
Making a histogram will be our first act of officially summarizing something about the data. We will no longer look at the individual bits of data, instead we will see how the numbers group together. \@fig-happyHist shows the same 500 happiness scores, this time grouped into bins. Each bar represents the number of people whose responses fall within a particular range. For example, the bar between 0 and 500 shows about 150 people reporting values in that range. The y-axis gives the frequency count—the height of each bar.
...
Plots and histograms give us a sense of where values fall, but they’re still rough. To be more precise, we can summarize many values with a single number that represents the “center” of the data. These are **measures of central tendency**, different ways of answering the question: *what are the values like, generally?*

### Mode

The **mode** is the most frequently occurring value in a dataset. That is it. How do you find it? You have to count the number of times each number appears, then whichever one occurs the most, is the mode.
...
Here, the mode is 1, but must of the numbers are a lot larger than that. The mode is not particularly useful for this dataset. Like any statistical tool, you need to think about whether the mode makes sense for your dataset and be ready to justify that choice.
...
The median is often a useful measure of central tendency becuase it stays put even if some values are extreme. For example:
