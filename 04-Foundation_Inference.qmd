...
If $H_0$ were true, individual measurements would follow the **null population distribution**: a population centered at $\mu_0$. Like any population, it has a partner: the **sampling distribution of the mean under $H_0$**, which describes what $\bar{x}$ would look like across many samples drawn from that null population. That partner is what replaces the simulated chance window. We meet the null population distribution in the next section and put its partner to work later in the chapter, when we test $H_0$.
...
- **Null population distribution.** The population distribution we would have *if $H_0$ were true*: centered at $\mu_0$, and as wide as the population itself. In the figures below it is the blue curve.
...
@fig-rejection-region shows how these ideas come together in a two-sided test. The blue curve is the partner promised earlier: the sampling distribution of the mean under $H_0$. Like the null population distribution it is centered at $\mu_0$, but it is narrower, with a spread of one standard error. It is the formal version of the chance window: instead of simulating the differences chance can produce, we assume a distribution and read them off it. The dashed magenta line marks the expected center at $\mu_0$. The red vertical lines are the **critical values** for $\alpha = 0.05$. They cut off the most extreme 5% of the distribution: 2.5% in each tail. Those shaded tails are the **rejection regions**.
...
@fig-ci-lens-illustrative shows another way to make the same decision. The **blue curve** is the null population distribution ($H_0$ true), and the **dark green curve** is the sampling distribution of the mean based on our sample. The **dashed green curve** is the estimated population distribution we infer from the sample.
...
ggplot() +
  ...
  annotate("text", x = -3.2, y = dnorm(-3.2, null_mean, null_sd) + 0.015,
           label = "Null population distribution", color = "blue", size = 3.6, hjust = 0, vjust=-0.4) +
