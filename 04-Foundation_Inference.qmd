...
plot_distributions <- function(null_mean, null_sd, sample_mean, sample_sd, inferred_mean, inferred_sd, true_mean, true_sd, show_ci = TRUE, show_samp = TRUE, y_top = 1.4, show_null = TRUE, show_true = TRUE) {
...
A test is about $\mu$, the true population mean, which we never observe; everything we know about it comes from one sample. The null hypothesis names a value for it, $\mu_0$. To reject $H_0$, the sample has to be hard to explain if the null were true: its mean, $\bar{x}$, has to land farther from $\mu_0$ than sampling error would plausibly carry it.

Several distributions and values take part in that comparison. @tbl-4players lists every one, with the line each is drawn with in this chapter's figures. Three are familiar from Chapter 3: the true population distribution, the estimated population distribution, and the sampling distribution of the sample mean. The rest come from the null hypothesis or are built later in the chapter.
...
players <- data.frame(
  line = "",
  player = c("$\\mu_0$", "Null population distribution", "True population distribution",
  ...
  what = c("The population mean if $H_0$ were true",
           ...
           "The population the data actually came from. Never known",
           "The sample mean: the one value we observe",
           "A normal curve from the sample's mean and standard deviation: our best picture of the true population",
           "Where $\\bar{x}$ would land across many samples: centered at $\\bar{x}$, spread = standard error",
           "$\\bar{x} \\pm 1.96$ standard errors, built from the sampling distribution of the sample mean (Route 2)",
           "Where $\\bar{x}$ would land if $H_0$ were true: centered at $\\mu_0$, spread = standard error (Route 1)")
)
swatches <- c(
...
kbl(players, escape = FALSE, col.names = c("Line", "Player", "What it is")) |>
  kable_styling(full_width = FALSE) |>
  column_spec(1, image = spec_image(swatches, 120, 40)) |>
  pack_rows("The hypothesis", 1, 2) |>
  pack_rows("The truth", 3, 3) |>
  pack_rows("The sample", 4, 7) |>
  pack_rows("For testing", 8, 8)
```

### Seeing them together

The players are easier to hold onto as a picture than as a table. Let's build that picture a curve or two at a time, ending at @fig-4key.keyfig.
...
p4 <-  plot_distributions(null_mean=0.5, null_sd=1, sample_mean=0.52, sample_sd=0.3, inferred_mean=0.52, inferred_sd=1, true_mean=0.54, true_sd=1, show_ci = FALSE, show_samp = FALSE, show_null = FALSE, y_top = 0.45)  # wide curves only: crop the empty space, same scale
print(p4)

```

**4. From individual readings to sample means.** Divide the sample's standard deviation by $\sqrt{n}$. (Not $\sqrt{n-1}$: the $n-1$ already went into computing the standard deviation itself, back in Chapter 3.) The result is the standard error, and it gives the **sampling distribution of the sample mean** (dark-green dot-dash): centered at $\bar{x}$, and much narrower. This special distribution is also where the confidence interval comes from, which we build later in this chapter.
...
p7 <- plot_distributions(null_mean=-0.2, null_sd=1, sample_mean=0.5, sample_sd=0.4, inferred_mean=0.5, inferred_sd=1, true_mean=0.6, true_sd=1, show_ci = FALSE, show_null = FALSE, show_true = FALSE)

print(p7)

```

**5. All together.** @fig-4key.keyfig puts the null population, the true population, the estimated population, and the sampling distribution of the sample mean in one picture. This is the figure you will see variations on for the rest of the book.
...
Critical values need one more curve, the last row of @tbl-4players: the **sampling distribution of the sample mean under $H_0$**, which describes what $\bar{x}$ would look like across many samples if $H_0$ were true. Its center, $\mu_0$, comes from $H_0$. Its spread is the standard error, $\sigma/\sqrt{n}$. This curve is the formal version of the chance window: instead of simulating the differences chance can produce, we assume a distribution and read them off it.
