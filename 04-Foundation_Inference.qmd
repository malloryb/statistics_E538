...
decision_panel <- function(xbar, se = 0.4, mu0 = 0) {
  xs_d <- seq(-4.5, 4.5, by = 0.01)
  crit <- 1.96 * se
  ...
  ggplot() +
    ...
    annotate("text", x = 4.45, y = 1.12, label = "'Under '*H[0]", parse = TRUE, hjust = 1, colour = "blue", size = 4.5) +
    annotate("text", x = -4.4, y = 1.12, label = "From the sample", hjust = 0, colour = "darkgreen", size = 4.5) +
    coord_cartesian(xlim = c(-4.5, 4.5), ylim = c(-0.08, 1.38)) +  # shared axes across the four-color figures
    labs(x = NULL, y = NULL) +
    theme_classic() +
    theme(axis.text = element_blank(), axis.ticks = element_blank())
}
