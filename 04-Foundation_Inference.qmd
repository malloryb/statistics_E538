...
**The two curves a decision uses.** Strip the picture back to the two sampling distributions of the sample mean (@fig-4two-sampling). Everything in the next section runs on these two: critical values and the $p$-value read off the one built from $H_0$, and the confidence interval comes from the one built from the sample.
...
#| fig-alt: "Two narrow dot-dash bell curves of the same width on shared axes: a blue one centered on a magenta dashed line labeled mu sub 0, and a dark-green one centered on a black dotted line labeled x-bar, to its right."
#| out-width: "100%"
...
ggplot() +
  ...
  annotate("segment", x = xbar_2, xend = xbar_2, y = 0, yend = 1.22, linetype = "dotted", linewidth = 1) +
  annotate("text", x = mu0_2 - 0.1, y = 1.3, label = "mu[0]", parse = TRUE, hjust = 1, colour = "magenta", size = 5) +
  annotate("text", x = xbar_2 + 0.1, y = 1.3, label = "bar(x)", parse = TRUE, hjust = 0, size = 5) +
