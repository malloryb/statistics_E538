...
possible_integers <- sample(1:10,20*10,replace=TRUE)
...
a<-sample(1:10,20*2,replace=TRUE)
...
a<-c(sample(1:10,20,replace=T),pmin(pmax(round(rnorm(20,5.5,1.6)),1),10))
df<-data.frame(a,sample=rep(paste("Distribution",1:2),each=20))
...
a<-c(sample(1:10,1000,replace=T),pmin(pmax(round(rnorm(1000,5.5,1.25)),1),10))
...
A few manipulations are so overwhelming that no statistics are required to see them. Flip a light switch up and the light comes on; flip it down and it goes off, every time, with no exceptions to weigh. Nobody runs a $t$-test on a light switch.

That case is worth naming precisely because nothing in environmental science behaves like it. Your effects are real signals buried in noisy, variable measurements, where flipping the switch changes the outcome *on average* and not in every instance. Inference is the machinery for that situation, and the rest of this chapter builds it.
...
To make this concrete, consider a scenario where we expect to find no real difference. A researcher surveys 10 caves for white-nose syndrome, a fungal disease that has killed millions of North American bats, capturing 20 bats at each cave and counting how many show visible signs of infection. @fig-5batdesign lays out the design.

```{r}
#| label: fig-5batdesign
#| fig-cap: "The bat-cave scenario as a study design. The split into two groups is arbitrary, which is the whole point: there is no real difference for the test to find."
...
Everything in this chapter asks one question: could chance alone have produced this result? That is the question this course answers, and it is the question behind every test in the chapters ahead. It is not the only question available. Other methods start from two or more candidate explanations, say a steady linear decline versus a sharp threshold response, and ask which one the data support better. Ruling out chance tells you that *something* is going on. Comparing models tries to tell you *what*. Those methods are beyond this book, but it is worth knowing the difference, because "significant" only ever answers the first question.
::::
...
Here's how the decision works. The grey band in the middle is the chance window: our 10,000 simulated replications produced differences from roughly `r round(diff_min,1)` to `r round(diff_max,1)` bats, and a result sitting well inside it, close to 0, is exactly what chance turns out routinely. No grounds for suspicion. The red zones on either side are the ones chance never reached, so a difference out there would give real grounds to suspect something beyond sampling error. Red marks the same thing here that it will in every later chapter: the region where you would reject chance as the explanation.

The gold bands are the honest part: differences near the edge of what chance can do, but not quite past it. Chance *could* produce something in the gold zone, it is technically still inside the window, but it does so rarely. That's not a clean yes/no; it's a "probably not chance, but I can't rule it out," and how you handle that ambiguity is a judgment call, not a fact. More conservative researchers shrink the gold zone (fewer false alarms, more missed real effects); more permissive researchers expand it (the reverse trade-off). Chapter 5 formalizes this trade-off with a standard convention ($\alpha = .05$) so that researchers aren't each inventing their own boundary from scratch.
...
Let's return to the bat-cave scenario, but now vary the number of sites surveyed per group: 5 (what we started with), 10, 20, and 40.
...
Let's say you are monitoring nitrogen dioxide (NO₂) concentrations at air quality sites near a major highway. Sites within 50 m of the highway go in the "near" group, and sites 500 m or more away go in the "far" group. If highway traffic is a meaningful source of NO₂, the near group should average higher concentrations. @fig-5no2design gives the design.

```{r}
#| label: fig-5no2design
#| fig-cap: "The highway NO2 study as a design schematic. Compare it with @fig-5batdesign: the same two-group structure, but here the grouping is a real difference rather than an arbitrary split."
#| fig-alt: "Study design schematic. IV: site position, categorical. Two boxes labelled Near highway and Far from highway, each with n = 20 sites. DV: NO2 concentration in ppb."
#| fig-asp: 0.34
#| out-width: "100%"

design_groups("Site position (categorical)", c("Near highway", "Far from highway"),
              20, "sites", "NO2 concentration (ppb)",
              "Do the two means differ by more than chance?")
```

That structure is worth pausing on, because it is identical to the bat-cave design in @fig-5batdesign: two groups, one measurement each, one question about the difference between their means. The only thing that changed is that the grouping now corresponds to something real. Everything this chapter does with the bats applies here unchanged.
...
Things could have turned out differently if some of the near-highway sites had landed in the far group instead, and vice versa. Shuffling the readings between groups is how we find out what "differently" looks like. The shuffle enacts the null hypothesis directly: if being near a highway makes no difference to NO₂, then the label on each reading is arbitrary, and we could swap the labels around without disturbing anything real. Every difference we get after shuffling is therefore a difference chance alone can produce.
...
`sample()` does the shuffling; the two lines after it simply deal the shuffled readings back out into groups of five. Notice that the ten readings themselves never change. All that changes is which group each one is assigned to, which is exactly the thing we are claiming does not matter.
...
```{r}
#| label: fig-5randtest
#| fig-cap: "Animation of a randomization test. Purple dots are the original condition means, yellow dots the means after each reshuffle, and the light green and red dots track how individual scores move between groups."
...
::: {.content-visible when-format="pdf"}
![Animation of a randomization test. Purple dots are the original condition means, yellow dots the means after each reshuffle, and the light green and red dots track how individual scores move between groups. (Final frame; the animated version is in the web edition of this book.)](imgs/gifs/stills/randomizationTest-1.png){#fig-5randtest width="75%"}
:::
