...
plot_distributions <- function(null_mean, null_sd, sample_mean, sample_sd, inferred_mean, inferred_sd, true_mean, true_sd, show_ci = TRUE, show_samp = TRUE, y_top = 1.4) {
...
::: {.callout-note title="Note: Why the curves from here on are normal"}
...
- **For a population**, it is an assumption. We never see the population, so we cannot know it is normal. What we can do is look at the sample: a histogram with no strong skew and no extreme outliers is consistent with a normal population. With a small sample this assumption carries more weight, because the Central Limit Theorem has less to work with.
...
The **null hypothesis**, $H_0$, is the claim that any gap between the sample and that value is sampling error: the population mean really equals it. We write that value as $\mu_0$. 

The **alternative hypothesis**, $H_a$, is the claim that the population mean does not equal $\mu_0$.
...
- **Estimated population distribution.** A normal curve built from the sample: centered at $\bar{x}$, with the sample standard deviation as its spread. It is what the sample suggests the population looks like. This is estimation from Chapter 3, viewed as a whole distribution rather than a single parameter. In a test it stands in for $H_a$.
...
- **Null population distribution.** The population distribution we would have *if $H_0$ were true*: centered at $\mu_0$, and as wide as the population itself. In the figures below it is the blue curve. It has a partner, the **sampling distribution of the sample mean under $H_0$**, which describes what $\bar{x}$ would look like across many samples drawn from that null population. That partner is what replaces the simulated chance window, and we put it to work later in the chapter when we test $H_0$.
...
The four are much easier to hold onto as a picture than as a list. Let's build that picture one curve at a time, ending at @fig-4key.keyfig.

Every curve in these figures is drawn as a normal curve. For the sampling distribution of the sample mean, that is the Central Limit Theorem at work. For the three population curves, it is an assumption: we treat the population as normal, and the estimated population is the normal curve with the sample's mean and standard deviation.

Here is what each line means in the figures that follow:
...
- **Light-green dashed line** = Estimated population distribution (built from the sample; our stand-in for $H_a$).
...
Two more elements join later in the chapter: a blue dot-dashed line for the sampling distribution of the sample mean under $H_0$, and a thick dark-green bar for the confidence interval.

**1. The null population distribution.**

Start with the null population distribution: the population we would have if $H_0$ were true. Its center, $\mu_0$ (magenta dashed line), comes from $H_0$ itself. Its spread is the population standard deviation, $\sigma$. In practice $\sigma$ is almost never known and the sample's standard deviation stands in for it; this chapter treats $\sigma$ as known as a simplification. The $t$-tests in Chapter 6 are the adjustment used in practice.
...
print(p_null + coord_cartesian(xlim = c(-4.5, 4.5), ylim = c(0, 0.45)))
```

**2. Null vs. true population.**

In an idealized world where $H_0$ is exactly true, the true population (orange) and the null population distribution (blue) are identical. In reality, we never know the orange curve.
...
**3. Adding the estimated population distribution.** We never know the true population distribution directly, but what we do have is a sample. From the sample's mean and standard deviation we build the **estimated population distribution** (light-green dashed). In the best case, this lines up closely with the true population (orange).

```{r}
#| label: fig-4key.perfectly_representative_sample
#| fig-height: 2.3
#| fig-cap: 'Perfectly representative sample: the estimated population distribution lines up with the true and null population distributions.'
#| fig-alt: "Three wide bell curves nearly stacked on top of one another near the same center: the blue null population distribution, the orange true population, and the light-green dashed estimated population."
#| out-width: "100%"

p4 <-  plot_distributions(null_mean=0.5, null_sd=1, sample_mean=0.52, sample_sd=0.3, inferred_mean=0.52, inferred_sd=1, true_mean=0.54, true_sd=1, show_ci = FALSE, show_samp = FALSE, y_top = 0.45)  # wide curves only: crop the empty space, same scale
print(p4)

```

**4. Adding the sampling distribution of the sample mean.** Divide the sample's spread by $\sqrt{n}$ and we get the **sampling distribution of the sample mean** (dark-green dot-dash): centered at $\bar{x}$, and much narrower. This special distribution is also where the confidence interval comes from, which we build later in this chapter.

```{r}
#| label: fig-4key.sample_imperfect_convenient
#| fig-cap: 'Imperfect but convenient sample: the estimated population sits close to the null population distribution, but not on it. The sampling distribution of the sample mean is centered on $\bar{x}$ and much narrower.'
#| fig-alt: "Four curves: the blue null population distribution sits somewhat left of the orange true population, light-green dashed estimated population, and dark-green sampling distribution, which are clustered together to its right. The null mean is marked with a dashed vertical line."
#| out-width: "100%"

p7 <- plot_distributions(null_mean=-0.2, null_sd=1, sample_mean=0.5, sample_sd=0.5, inferred_mean=0.5, inferred_sd=1, true_mean=0.6, true_sd=1, show_ci = FALSE)
...
Key reminder: $\mu_0$ (magenta dashed line) comes from the null hypothesis. The unknown $\mu$ is what we're trying to learn about, and we approximate it from the sample through the estimated population distribution and the sampling distribution of the sample mean.

```{r}
#| label: fig-4key.keyfig
#| fig-cap: 'Key elements of hypothesis testing graphs: null population (blue), true population (orange), estimated population (light-green dashed), sampling distribution of the sample mean (dark-green dot-dash), and null mean $\mu_0$ (magenta dashed).'
#| fig-alt: "The reference legend figure: a blue null population distribution on the left, a dashed vertical line marking its mean mu sub 0, and to the right a cluster of three curves close together, the orange true population, the light-green dashed estimated population, and the narrow dark-green dot-dash sampling distribution of the sample mean, clearly separated from the null mean."
#| out-width: "100%"

pkey <-  plot_distributions(null_mean=-0.5, null_sd=1, sample_mean=0.5, sample_sd=0.3, inferred_mean=0.5, inferred_sd=1, true_mean=0.75, true_sd=1, show_ci = FALSE)
print(pkey)
```
...
|---------------------|---------------------------|----------------------|--------------------|
| **From the sample** | Estimated population distribution, centered at $\bar{x}$ | Sampling distribution of the sample mean, centered at $\bar{x}$ | Confidence interval |
| **Under $H_0$**     | Null population distribution, centered at $\mu_0$ | Sampling distribution of the sample mean under $H_0$, centered at $\mu_0$ | Critical values, $p$-value |
...
@fig-rejection-region shows how these ideas come together in a two-sided test. The blue dot-dash curve is the partner promised earlier: the sampling distribution of the sample mean under $H_0$. Like the null population distribution it is centered at $\mu_0$, but it is narrower, with a spread of one standard error. It is the formal version of the chance window: instead of simulating the differences chance can produce, we assume a distribution and read them off it. The dashed magenta line marks the expected center at $\mu_0$. The red vertical lines are the **critical values** for $\alpha = 0.05$. They cut off the most extreme 5% of the distribution: 2.5% in each tail. Those shaded tails are the **rejection regions**.
...
#| fig-cap: 'Two-sided test: blue dot-dash curve is the sampling distribution of the sample mean under $H_0$ (centered at $\mu_0$). Magenta dashed line marks $\mu_0$. Red vertical lines mark the critical values for $\alpha=0.05$; shaded tails are the rejection regions.'
