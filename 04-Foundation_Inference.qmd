...
Because we used the uniform distribution to create these numbers, we already know where they came from. But suppose someone handed you this same list without telling you its source: could you tell, just by looking, that it came from a uniform distribution? You'd need to check is whether all the numbers occur with roughly equal frequency, since that's what a uniform distribution predicts: if each number had the same chance of occurring, each should show up roughly the same number of times.

Putting the sample of 100 numbers into a histogram makes the counts easy to compare. If all values from 1 to 10 occur with equal frequency, each individual number should occur about 10 times. @fig-5histunif shows the histogram:
...
```{r}
#| label: fig-5gumballA
#| fig-cap: "Mean pH for Group A and Group B sampled randomly from the same stream population. The y-axis is zoomed in (it doesn't start at 0) so the difference between bars is visible; pH itself doesn't range down to 0 in a real stream."
...
The first step is to replicate the sampling scenario many times. (ah I see, this used to be gumballs, but now it's PH for "environmental science" reasons. Tbh the PH isn't working becuase again it's log scale. I think something that's at least a discrete # of things. Like maybe, how many bats in your sample have white nose fungus, or need to think of some other idea that is nicer than Ph for this example) @fig-5gumballsims shows bar graphs of mean pH for Group A and Group B across 10 replications of the same experiment: all 10 sites sampled from the same population, split 5+5, and means computed.
...
The differences in these 10 replications mostly fall in a narrow range. To better characterize what chance can do, we run 100 replications. The results are shown in @fig-5manydiffs.(The #s are all smushed in the below figure's x-axis)
...
We already have everything we need to make this concrete: the pH simulation above gave us 10,000 mean differences that chance alone can produce when there is no real difference between groups. That histogram (@fig-5histdiffgumball) *is* our reference distribution. Now we use it to make a decision.
...
The boundaries are the minimum and maximum values chance actually produced across the 10,000 replications. @fig-5simdecision shows this applied to our pH differences.

```{r}
#| label: fig-5simdecision
#| fig-cap: "Decision boundaries applied to the histogram of pH mean differences from @fig-5histdiffgumball. Green marks differences chance never produced in 10,000 replications; red marks the range chance did produce; grey marks a zone chance produced only rarely."
...
ggplot(data.frame(differences), aes(x = differences)) +
  ...
  xlab("Mean difference in pH (Group A - Group B)") +
  ylab("Count")

```

Here's how the decision works. A pH difference beyond the green boundary (roughly `r round(diff_min,2)` to `r round(diff_max,2)`, from our 10,000 simulated replications) never happened by chance in this simulation, so observing something out there would give real grounds to suspect it wasn't just sampling error. A difference well inside the red zone, close to 0, is exactly what chance produces routinely: no grounds for suspicion.
...
#| fig-cap: "The width of the chance window for mean pH differences shrinks as the number of sites per group increases."

ns <- c(5, 10, 20, 40)
sims <- 5000
all_df <- data.frame(
  sample = c(sapply(ns, function(x) replicate(sims, mean(rnorm(x, 7.0, 0.4)) - mean(rnorm(x, 7.0, 0.4))))),
...
ggplot(all_df, aes(x = sample)) +
  ...
  ggtitle("Chance window for pH mean differences, by sites per group") +
  ylab("Count") +
  xlab("Mean difference in pH (Group A - Group B)")
...
```{r}
#| label: tbl-5minmax
#| tbl-cap: "Smallest and largest pH mean differences produced by chance, by sites per group."
...
**Important:** **what could have happened** always means what could have happened **by chance**. Comparing what did happen to what chance could have done is how we assess whether a result is likely due to chance or something real.
...
These pictures do real work: they summarize a pile of complicated numbers into a single inference, without even needing a summary number, just look at whether the observed difference falls inside or outside the chance window. Going forward, we'll formalize this process into "standard" inferential statistics, replacing the picture with a summary number: the probability that the observed difference could have been produced by chance. A 95% probability would leave little confidence that the environmental condition was causing the difference; a 1% probability would give much more confidence that **chance did not** produce it, and that the variable of interest (highway proximity, pollution source, land use type) actually matters. Getting to that number is what the next few chapters build toward.
