...
Both put a distance on top and divide by the spread of the curve you are standing on. What changes is the curve. A $z$-score places one value on the population distribution, where the spread is $\sigma$. A $z$ statistic places $\bar{x}$ on the sampling distribution of the sample mean under $H_0$, which is narrower, so the spread is $SE = \sigma/\sqrt{n}$. Dividing $\bar{x} - \mu_0$ by $\sigma$ instead of $SE$ is a mistake to watch for: it uses the wide curve to answer a question about the narrow one.
