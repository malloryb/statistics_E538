...
players <- data.frame(
  ...
  what = c("The population mean if $H_0$ were true",
           ...
           "Where $\\bar{x}$ would land across many samples if $H_0$ were true: centered at $\\mu_0$, spread = standard error. Critical values come from it (Route 1)",
...
**Notation reminder.** The labels use the shorthand from Chapter 3. $X \sim N(\mu, \sigma^2)$ reads "the random variable $X$ is normally distributed with mean $\mu$ and variance $\sigma^2$." The second number is the variance, the square of the standard deviation, so $N(\mu_0, SE_{\bar{x}}^2)$ is a normal curve centered at $\mu_0$ whose standard deviation is the standard error.
