...
#[Should I consider taking making these colors different or more similar to what I showed in Ch 3? I like them as is but also want consistency]
...
Everything so far was built by brute force: split the sites into groups at random ten thousand times, collect the difference each time, and look at where a real result falls against that pile. As you can imagine, this is not practical most of the time. [Transition here is really rough. Think we'll need to make it smoother and also it should include tying the core players back to where we discussed them in the preivous chapter when possible. Also wondering if "parent" is too confusign or if it's ok]
