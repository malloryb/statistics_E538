...
- **For a population**, it is an assumption. We never see the population, so we cannot know it is normal. What we can do is look at the sample: a histogram with no strong skew and no extreme outliers is consistent with a normal population. The assumption matters most with small samples. The Central Limit Theorem only promises a normal shape for sample means once the sample is large, so with a small sample the normal curve for $\bar{x}$ rests on the population itself being roughly normal.
