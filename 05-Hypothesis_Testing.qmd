...
# NOTE: this is Chapter 4's plot_distributions(), copied verbatim so both
# chapters draw the same curves with the same names, colors and scale.
# Two copies is the known cost; consolidate into global_stuff.R next year.
...
-   In @fig-5key.type1_error, the true population distribution (orange) is almost identical to the null population distribution (blue). By chance, however, the sample mean lands far away, and the CI excludes $\mu_0$. The test rejects $H_0$ even though it is true: a **Type I error**.

```{r}
#| label: fig-5key.type1_error
#| fig-cap: 'Type I error: $H_0$ is true ($\mu=\mu_0$), but the sample leads us to reject $H_0$.'
#| fig-height: 3.96
#| fig-alt: "Four overlaid normal curves: a blue null population distribution and an orange true population distribution sit almost on top of each other, both centered near negative 0.5, while a dashed estimated-population curve and a dot-dash sampling-distribution curve are centered far to the right, near 2. A thick horizontal bar marking the 95 percent confidence interval sits around that same right-shifted location and does not reach back to the null mean, marked with a dashed vertical line, so the interval excludes the null value even though the true and null distributions are nearly identical."
#| out-width: "100%"

p8 <- plot_distributions(null_mean=-0.5, null_sd=1, sample_mean=2, sample_sd=0.5, inferred_mean=2, inferred_sd=1, true_mean=-0.52, true_sd=1, y_top = 1.05)  # crop the empty space, same scale as Ch4
print(p8)
```

-   In @fig-5key.type2_error, the true population mean (orange) is far from $\mu_0$, but the particular sample looks close to the null population distribution. The CI includes $\mu_0$, so the test fails to reject when it should reject the null: a **Type II error**.
...
#| fig-height: 3.96
#| fig-alt: "Four overlaid normal curves: a blue null population distribution centered near negative 0.5, an orange true population distribution centered far to the right near 3, and a dashed estimated-population curve with a dot-dash sampling-distribution curve both centered near 0, much closer to the null population distribution than to the true population distribution. A thick horizontal bar marking the 95 percent confidence interval sits near 0 and reaches back to include the null mean, marked with a dashed vertical line, so the interval fails to exclude the null value even though the true population is far away."
#| out-width: "100%"

p9 <- plot_distributions(null_mean=-0.5, null_sd=1, sample_mean=0, sample_sd=0.4, inferred_mean=0, inferred_sd=1, true_mean=3, true_sd=1, y_top = 1.05)  # crop the empty space, same scale as Ch4
print(p9)
```
