...
@fig-5alpha-beta does that. The blue curve is the **sampling distribution of the sample mean under** $H_0$, the one that sets the critical values. The orange curve is new here: the **sampling distribution of the sample mean if** $\mu = \mu_a$, meaning the population mean really is some particular value $\mu_a$ other than $\mu_0$. There is one such curve for every value $\mu_a$ could take, and the miss rate depends on which one is true: the larger the real difference, the harder it is to miss.
