...
In @fig-5alpha-beta, $\mu_a$ sits 2.5 standard errors above $\mu_0$, which leaves $\beta = 0.29$ and power $= 0.71$: this design misses a real difference of that size in almost three tries out of ten. Slide the orange curve further right, meaning a larger real difference, and the gray area shrinks. Move the red lines outward, meaning a smaller $\alpha$, and the gray area grows. That is the tradeoff the table above states in words.
...
**The raw difference has a name when you are planning.** Designing a study means asking how often you would catch a difference of a given size. The size you ask about is the raw distance between the null value and the candidate value:
