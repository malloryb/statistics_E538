...
@fig-5alpha-beta does that. The blue curve is the **sampling distribution of the sample mean under** $H_0$, the one that sets the critical values. The orange curve is new here: the **sampling distribution of the sample mean if** $\mu = \mu_a$, meaning the population mean really is some particular value $\mu_a$ other than $\mu_0$. There is one such curve for every value $\mu_a$ could take, which is why the Type II error rate is written $\beta(\mu_a)$. The miss rate depends on how large the real difference is.
