...
#| fig-cap: 'The two error rates, drawn on sampling distributions. Top: $\alpha$ is the red area in the tails of the null curve. Bottom: adding the curve for one specific alternative value $\mu_a$, $\beta$ is the gray area under it between the critical values, and power is the orange area past the right critical value.'
#| fig-height: 6.6
...
plab_ab <- annotate("text", x = 3.55, y = .30, label = "power", color = "darkorange3", size = 4.5)
...
#| fig-cap: 'The Type II situation at two sample sizes, with the same $\mu_0$, true $\mu$ and population. In both, $\bar{x}$ lands 2.5 standard errors below $\mu$. Top: the confidence interval catches $\mu_0$, so we fail to reject. Bottom: with four times the sample, the standard error halves, the same bad luck leaves the interval clear of $\mu_0$, and we reject.'
...
-   **Practical significance**: the effect is large enough to change what someone does about it. This is a judgment about the world, not a calculation. A result can be statistically significant and practically trivial, which is what a large enough $n$ buys you, or practically important and statistically undetected, which is what too small an $n$ costs you.
...
::: {.callout-note title="When you actually do this"}
