...
# The figure captions quote these numbers; change them together.
...
t2_power    <- 1 - (pnorm(1.96 - t2_mu_se) - pnorm(-1.96 - t2_mu_se))
...
error_panel <- function(xbar, mu, se = err_se, sigma = err_sigma, mu0 = 0,
  ...
  ggplot() +
    ...
    annotate("segment", x = xbar, xend = xbar, y = -0.04, yend = 1.22, linetype = "dotted", linewidth = 1) +
    ...
    annotate("text", x = xbar + 0.08, y = if (abs(xbar - mu) < 0.5 || abs(xbar - mu0) < 0.5) 1.18 else 1.3,
             label = "bar(x)", parse = TRUE, hjust = 0, size = 5) +
...
Errors happen because samples are imperfect. @fig-5key.type1_error and @fig-5key.type2_error are the reject and fail-to-reject figures from Chapter 4 with one addition: the truth, in orange. The blue curve and its red tails are the sampling distribution of the sample mean under $H_0$, which sets the critical values. The green curve is the sampling distribution of the sample mean built from the sample, and the green bar is its confidence interval. The orange curve is the true population distribution, and the orange line marks its mean, $\mu$. With real data we never see the orange. We draw it here so the decision can be checked against it.

-   In @fig-5key.type1_error, $H_0$ is true: $\mu$ sits exactly on $\mu_0$, so the orange and magenta lines coincide. By chance the sample mean lands `r t1_xbar_se` standard errors from $\mu$, past the critical value, and the confidence interval excludes $\mu_0$. The test rejects a true $H_0$: a **Type I error**. A sample mean lands that far from $\mu$, in one direction or the other, only about `r round(200 * pnorm(-t1_xbar_se))`% of the time.

```{r}
#| label: fig-5key.type1_error
#| fig-cap: 'Type I error. $H_0$ is true ($\mu = \mu_0$), but $\bar{x}$ lands 2.3 standard errors out, past the critical value, and the confidence interval excludes $\mu_0$.'
#| fig-alt: "A blue dot-dash bell curve centered on a magenta dashed line labeled mu sub 0, with red critical-value lines and shaded tails. A solid orange line labeled mu sits exactly on the magenta line, and a wide, low orange bell curve is centered there too. A dark-green dot-dash bell curve of the same width as the blue one sits to the right, centered on a black dotted line labeled x-bar that lies just past the right critical value. The thick dark-green confidence interval bar under it starts just to the right of mu sub 0, so it excludes mu sub 0."
#| out-width: "100%"

error_panel(xbar = t1_xbar_se * err_se, mu = 0)
```

-   In @fig-5key.type2_error, $H_0$ is false: $\mu$ sits `r t2_mu_se` standard errors above $\mu_0$, far enough that this design would catch a difference that size about `r round(100 * t2_power)`% of the time. This sample mean happened to land `r t2_mu_se - t2_xbar_se` standard errors below $\mu$, which leaves it inside the critical values, and the confidence interval includes $\mu_0$. The test fails to reject a false $H_0$: a **Type II error**.

```{r}
#| label: fig-5key.type2_error
#| fig-cap: 'Type II error. $\mu$ is 3.5 standard errors above $\mu_0$, but $\bar{x}$ lands 2 standard errors below $\mu$, inside the critical values, and the confidence interval includes $\mu_0$.'
#| fig-alt: "A blue dot-dash bell curve centered on a magenta dashed line labeled mu sub 0, with red critical-value lines and shaded tails. Well to the right, a solid orange line labeled mu marks the center of a wide, low orange bell curve. A dark-green dot-dash bell curve of the same width as the blue one sits between them, centered on a black dotted line labeled x-bar that lies inside the right critical value. The thick dark-green confidence interval bar under it reaches back past mu sub 0, so it includes mu sub 0."
#| out-width: "100%"

error_panel(xbar = t2_xbar_se * err_se, mu = t2_mu_se * err_se)
```

In both cases the test worked exactly as designed. What went wrong is the sample: its mean landed about 2 standard errors from the true mean, the same amount of bad luck in opposite situations.
