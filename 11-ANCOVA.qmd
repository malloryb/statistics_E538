...
As we've explored statistical modeling, we've understood the power of both regression and ANOVA. Now, let's merge them:

$$\text{Regression} + \text{ANOVA} = \text{ANCOVA}$$


The last chapter compared macroinvertebrate species count across two discrete Pollution categories, present or absent (actually think I said "no pollution" or "pollution" but this is probably better lingo), and two Flow categories, low or high. But (could also say that sadly no pollution is a pipe dream in most cases, but maybe too much of an aside, or also that "high" and "low" are hard, where did we make hte cutoff, etc, might as well just measure continuously if we can, or something practical to make that argument?) suppose you actually measured each channel's pollution concentration instead of just labeling it present or absent. 
...
Each question pairs a continuous relationship, the covariate's (don't I define a covariate later? that doesn't work then) effect on the outcome, with a categorical grouping. That pairing, one continuous covariate and one categorical factor, is what makes a design an ANCOVA rather than a plain regression or a plain ANOVA.

The middle question is worth sitting with, because it's really a complaint about the last chapter. "Pollution: present or absent" is a real simplification of something that's actually continuous, a concentration in parts per billion, measured on a gradient, not a light switch (ah ok here's what I was asking for already!). Sorting continuous measurements into "high" and "low" buckets throws away exactly the information a covariate needs: how much more polluted, not just whether. Whenever you have the real numbers, ANCOVA (or, if there's no categorical factor left at all, plain regression) is almost always the better tool than forcing them into groups first.

ANCOVA is a linear model built directly on regression's own equation. A simple linear model is

$$y = \beta_0 + \beta_1 x + \varepsilon$$

(Next sentence sounds AI-y, just say where beta_0 is intercept, B1 is the line, and epsilon is the error, and this is one line). one intercept, one slope, one line. 

ANCOVA adds a categorical factor to that equation, and the whole chapter turns on (turns on? weird phrasing) one question: does every group follow that same line, or does each group need its own slope and intercept? 
...
@fig-11onevstwo shows the three possibilities, and it's worth keeping this picture in mind for the rest of the chapter.
...
(yes covariate all the way down here. concerned about that with the begining) 
...
The categorical predictor itself, in every example this chapter uses, is what's called a **fixed factor**: specific, intentional levels chosen by the researcher (urban vs. rural, shaded vs. open canopy), not a random sample of possible groups. That distinction matters more once you're building mixed-effects models, which is beyond where this course goes. (But, you could imagine a situation where, say, your streams are distributed across 10 different counties....(something better for example of random effect?) different )

## What ANCOVA Is (is this the best heading here?)
...
(check also consolidated/standardized callouts yesterday adn I don't think I usually had "tip' or a header for these)
::: callout-tip
**The other way you'll see ANCOVA.** Elsewhere, the covariate is often a **nuisance** instead: something influencing the outcome that you control for so a group comparison comes out fairer, and an interaction is a box you check and hope stays empty. 

A classic version from psychology: comparing anxiety scores across two therapy groups at post-treatment, controlling for each participant's pre-treatment score as the covariate. 

Nobody there wants to find an interaction (I mean who knows maybe they do, is there an even cleaner example?); they want a clean, adjusted comparison of the two treatments. 

It's the same test (and the same math), the only difference is which term you came to interpret, so decide that before you fit anything.
:::
...
Before we can read real coefficients off an ANCOVA table, it helps to see exactly how one equation can represent two different groups at once. That's a short detour, and it pays off immediately below.
...
Consider two predictors: a continuous variable $x_l$ and an indicator $x_c$ (0 or 1). Use this model: (should I actually use example for the indicator from examples we've already discussed? it would then be xx indicator on?)
...
(Think the mechanism part should be cut - this whole 2nd paragraph in the callout - because this mechanism is about the dummy variables but the GLM unification point isn't. necessary and it's not being made very clearly in my opinion)
This same mechanism underlies every test in this book: $t$-tests, ANOVA, regression, and ANCOVA are all the same linear model with different kinds of predictors. Chasing that unification all the way, through reference coding across many categories, planned contrasts, and several continuous predictors at once, is what a second course in regression is for. See the out-of-sequence chapter *"The Linear Model Framework: Dummy Variables and Interactions"* in this book's repository if you want to go there.
:::

### The Null Hypothesis in ANCOVA
(ok I called indicator x_c above and now calling it g so it needs to be standard)

**Model.** For one covariate (x) and a two-level group indicator $g\in{0,1}$,

$y=\beta_0+\beta_1 x+\beta_2 g+\beta_3(x\times g)+\varepsilon .$

**Step 1: Are slopes equal across groups?** This tests whether the covariate effect is the same in each group. (Think the 3 models figure elftmost to rightmost might make more sense down here than way up where it is in teh very beginning of chapter )
...
::: callout-note
**ANCOVA assumptions.** Most of these are ones ANOVA already needed; two are new because a covariate has entered the model.
...
-   Homogeneity of slopes: the groups share that same line, unless the interaction test says otherwise *(new --- this is the one the interaction test above directly checks), this isn't required to run the ANCOVA it's what the ANCOVA checks...*
-   The covariate is measured without substantial error (isn't that true for everythign?) and isn't itself affected by the treatment *(new, about the covariate, but isn't that true for every independent variable? I don't htink this one seems like a "real" one)*
:::
...
We might expect that air pollution will have a stronger negative effect in urban areas than in rural areas (why would we expect that - need simple explanation)
...
(make note/aside about how notice R handles it automatically, it's the same aov call we've been using)
...
So we interpret the omnibus test through these individual terms rather than as a separate $p$-value. (So this test doesn't have it's own p-value in teh table. Make that clearer)
...
(check on the chapter summary we did befoer but this key takeaways should maybe be in that part right?)
**Key takeaways**
