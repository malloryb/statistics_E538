...
-   Are dbh (Diameter at Breast Height) and height related similarly for tulip poplars and oaks? *(covariate: dbh; factor: species)*

-   The last chapter compared macroinvertebrate species count across two discrete Pollution categories, present or absent, and two Flow categories, low or high. But suppose you actually measured each channel's pollution concentration instead of just labeling it present or absent. Does Flow's effect on species count still depend on pollution once pollution is treated as the continuous measurement it really is, rather than binned into two groups? *(covariate: pollution concentration; factor: flow rate)*
...
Each question pairs a continuous relationship, the covariate's effect on the outcome, with a categorical grouping. That pairing, one continuous covariate and one categorical factor, is what makes a design an ANCOVA rather than a plain regression or a plain ANOVA.
...
one intercept, one slope, one line. ANCOVA adds a categorical factor to that equation, and the whole chapter turns on one question: does every group follow that same line, or does each group need its own slope and intercept? We answer it in two steps: first test whether groups have different **slopes**, then, only if slopes are parallel, test whether groups differ in **adjusted means**.
...
::: callout-tip
**The other way you'll see ANCOVA.** Elsewhere, the covariate is often a **nuisance** instead: something quietly influencing the outcome that you control for so a group comparison comes out fairer, and an interaction is a box you check and hope stays empty. A classic version from psychology: comparing anxiety scores across two therapy groups at post-treatment, controlling for each participant's pre-treatment score as the covariate. Nobody there wants to find an interaction; they want a clean, adjusted comparison of the two treatments. Same test, same math. The only difference is which term you came to interpret, so decide that before you fit anything.
:::
...
-   Homogeneity of slopes: the groups share that same line, unless the interaction test says otherwise *(new --- this is the one the interaction test above directly checks)*
-   The covariate is measured without substantial error, and isn't itself affected by the treatment *(new, about the covariate)*
:::
