...
As we've explored statistical modeling, we've understood the power of both regression and ANOVA. Now, let's merge them. Analysis of Covariance (ANCOVA) allows us to examine relationships across different categories.

(format more nicely)
Regression + ANOVA = ANCOVA


Consider these questions:(make better, don't like the BTUs one)
...
(Add scaffolding noting the continuous covariate, and the categorial factor for prior examples). 

(I don't know that this callout note is helpful)
::: callout-note
...
(redundancy from here....)
ANCOVA is a linear model with one continuous covariate and one categorical factor; it tests group differences **after** adjusting for the covariate and, first, whether slopes differ across groups.
...
### Terminology in LMs (can this not be cut at this point? maybe we should keep it)
...
**Analysis of Covariance (ANCOVA)** extends ANOVA by adding a continuous predictor. It fits a linear model with both a categorical factor and a covariate, allowing us to compare groups **and** account for how the response changes with the continuous variable. (this is also redundant with earlier in the chapter)

(Now things are finally becomming a little different)
...
(The callout is too long. Just say something like: Here, we're presenting ANCOVA as a way to answer questions about how predictors function with respect to each other. Other presentations of ANCOVA sometimes present the covariate as a nuisance (your expalantion is fine), which means they're trying to make sure there sin't an interaction. Thhey're the same test, you're just changing what term you try to interpet. And give an example here, maybe one from psychology since that seem sto be a common thing. So nuiscance variable, ad n question-driven framing.)
...
### Indicator (dummy) variables and how they work (this feels a little out of order maybe, or maybe it's fine if the detour is signposted at the beginning like it is at the end with the callout)
...
**Why this matters.** ANCOVA asks two questions in order. First, do groups have the **same slope** for the covariate--response relation. Only if slopes are the same do we ask whether groups differ in **adjusted means** after controlling for the covariate.(we've said this multiple times already)
...
When we run an ANCOVA, the output includes an **omnibus F-test**, a single test of whether there are any differences in the adjusted group means after accounting for the covariate. (omg wasn't this already explained?)
...
(Why is hoogeneity of slopes bolded? Indicate which are new for ANCOVA, the same for ANOVA, and the covariate one, explain better)
...
R automatically converts the categorical `AreaType` variable into a 0/1 indicator behind the scenes.(tie back to the dummy variable detour) It picks one level (here, "Rural," alphabetically first) as the **reference**, and every coefficient involving `AreaType` is interpreted relative to that reference. That's why you see a coefficient called `AreaTypeUrban` but no separate `AreaTypeRural` term.
...
Centering does **not** change the slopes or their $p$-values. (just said this)
