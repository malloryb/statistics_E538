...
knitr::opts_knit$set(allow_html = TRUE)  # Allow HTML inside a PDF build (prevents early abort)

# 1) Mask kable() so it auto-chooses format per target
kable <- function(x, format = NULL, ...) {
  if (knitr::is_latex_output()) format <- "latex" else if (is.null(format)) format <- "html"
