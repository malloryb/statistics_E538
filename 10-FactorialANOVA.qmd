...
Under low flow, species count drops sharply with pollution (`r f1(mean(A))` to `r f1(mean(B))`, a difference of `r f1(simple_low)`). Under high flow, the drop is smaller (`r f1(mean(C))` to `r f1(mean(D))`, a difference of `r f1(simple_high)`). The effect of pollution depends on flow rate.
...
If the main effects told the whole story, each cell mean would be the grand mean, shifted by that cell's pollution effect and its flow effect. Take the no-pollution, low-flow cell. The no-pollution marginal mean is $`r round(nopoll_mean,2)` - `r round(gm,2)` = `r round(nopoll_mean - gm,2)`$ above the grand mean, and the low-flow marginal mean is $`r round(gm,2)` - `r round(low_mean,2)` = `r round(gm - low_mean,2)`$ below it, so the two main effects predict $`r round(gm,2)` + `r round(nopoll_mean - gm,2)` - `r round(gm - low_mean,2)` = `r round(nopoll_mean + low_mean - gm,2)`$. The observed cell mean is `r f1(mean(A))`. The leftover, `r round(mean(A) - (nopoll_mean + low_mean - gm),2)`, is the part of that cell the two main effects can't explain. Those leftovers, one per cell, are what the interaction captures.
...
We'll create bar and line graphs to illustrate these patterns. Bar graphs are great for seeing differences in means directly, while line graphs help us spot interactions. @fig-10bar22 shows the possible patterns in bar graph form. These are abstract illustrations of every combination that can occur, so a few of them describe patterns you would not expect ecologically. Use them to learn to recognize the shapes.
...
Those two numbers describe the interaction: pollution costs `r f1(simple_low)` species under low flow and `r f1(simple_high)` under high flow. But saying "pollution reduced species count under low flow" is still a claim, and like any claim in this book, it needs its own test, not just a look at the plot.
...
@fig-10mainint shows both a main effect and an interaction. Unpolluted channels are above polluted ones at both flow rates. That is the main effect. But the size of that gap is larger under low flow (12 vs. 5) than under high flow (14 vs. 11), so there's an interaction too.
...
### An "averaged" main effect driven by an interaction
