...
ggplot(soil_cells, aes(Soil, Water)) +
  geom_tile(aes(fill = I(fill)), color = "grey30", linewidth = 0.6, width = 0.92, height = 0.92) +
  geom_text(aes(label = paste0(Soil, "\n", Water)), fontface = "bold", size = 4.1, vjust = -0.15, color = "grey15") +
  geom_text(aes(label = "DV measured here"), size = 3.2, vjust = 2.1, color = "grey30") +
...
Could use the Icons up here too. 
...
**The design.** Our two IVs are pollution ("No Pollution" vs. "Pollution") and flow rate ("High Flow" vs. "Low Flow"). Two IVs, two levels each — you should already be able to name this factorial design. It's a 2x2.
...
(remove snail and bug labels from the figure, it pushes up the Pollution and high flow labels above the boxes)
```{r}
#| label: fig-10setup
#| fig-cap: "The 2x2 experimental design. Two pollution conditions crossed with two flow rates gives four stream-channel conditions. In every condition we count how many macroinvertebrate species are present (the bug and snail icons), pollution is present or absent (the factory icon), and flow rate is marked by the number of water drops."
#| fig-alt: "A two-by-two grid. Columns are labeled No Pollution and Pollution; rows are labeled Low Flow and High Flow. Each of the four cells notes five stream channels, shows a factory icon only in the two Pollution cells, shows one water drop in the two Low Flow cells and three water drops in the two High Flow cells, and shows a caterpillar and snail icon marking the dependent variable in every cell."
#| out-width: "90%"
#| fig-asp: 0.62
...
cells$dv_icon <- "\U0001F41B \U0001F40C"

p_setup <- ggplot(cells, aes(Pollution, Flow)) +
  geom_tile(aes(fill = I(fill)), color = "grey30", linewidth = 0.6, width = 0.92, height = 0.92) +
  geom_text(aes(label = paste0(Pollution, "\n", Flow)),
            fontface = "bold", size = 4.0, vjust = -1.5, color = "grey15") +
  geom_text(aes(label = pollution_icon), size = 7, vjust = 0.2) +
  geom_text(aes(label = flow_icon), size = 5, vjust = 1.4) +
  geom_text(aes(label = dv_icon), size = 6.5, vjust = 3.1) +
...
(think a callout here, or near here, about how to know which to put on the y and which to put on the x for these plots would be helpful. In short, it doesn't really matter, but sometimes it's more intuitive to do it one way than another. Maybe have domse lecture notes ont his one, remember vaguely)
...
(Missed note here: Need to check in the whole book but think this is rendering obnoxiously to give the 3rd header level its own numbers in this chapter, at least). 
...
(Is why it matters standard for summaries? Worked yestedrday to standardize these across book but don't recall core ideas or why it matters being in there)
