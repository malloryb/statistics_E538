...
```{r}
#| label: fig-10designs
#| fig-cap: "Structure of a 2x2 factorial design: soil type crossed with water availability."
#| fig-alt: "A two-by-two grid. Columns are labeled Clay and Loam; rows are labeled Low Water and High Water. Each of the four cells is marked DV, indicating that the dependent variable is measured in every combination."
#| out-width: "75%"
#| fig-asp: 0.55

soil_cells <- expand.grid(
  Soil  = factor(c("Clay", "Loam"), levels = c("Clay", "Loam")),
...
ggplot(soil_cells, aes(Soil, Water)) +
  ...
  labs(x = "Soil type", y = "Water availability") +
...
**The question.** Streams are affected by pollution, but they are also affected by how fast the water moves. Fast-moving water flushes contaminants downstream and re-oxygenates the channel; slow water lets them sit. So we might reasonably suspect that pollution does more damage in a sluggish stream than in a fast one.

Before we look at any data, it's worth stating that suspicion as a real prediction, because a factorial design is what lets us actually test it. We'd guess that no pollution plus high flow supports the most species, and that pollution plus low flow supports the fewest. But the other two combinations are the interesting ones, and a one-factor study could never distinguish them. Is a polluted, fast-moving stream more like a healthy stream, because the flow washes the pollution through before it can do damage? Or is it more like a degraded one, because the pollution is still there regardless of flow? Put differently: does pollution matter only when flow is low, with fast water washing its effect away entirely, or does pollution hurt a stream no matter how fast the water moves? That is exactly the kind of question an interaction test answers.
...
Before running the full machinery, it's worth seeing what the interaction term is actually testing, because the answer is less mysterious than the arithmetic makes it look. Recall from earlier: an interaction is a **difference between differences**. Here's that idea with real numbers, using the four condition means directly, before we touch a single sum of squares.
...
df_total <- rbind(df_total, c("Sums", "", sum(species), sum(diff), sum(diff_sq)))
...
df_poll <- rbind(df_poll, c("Sums", "", sum(species), "", sum(diff), sum(diff_sq)))
...
df_flow <- rbind(df_flow, c("Sums", "", sum(species), "", sum(diff), sum(diff_sq)))
...
df_int <- rbind(df_int, c("Sums", "", sum(species), "", sum(diff), sum(diff_sq)))
