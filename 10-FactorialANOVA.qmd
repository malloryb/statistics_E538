...
A factorial design has more than one IV, with every level of each IV fully crossed with every level of the others. This chapter covered the computation: analyzing a repeated-measures 2x2 design with paired-samples $t$-tests and confirming it against R's ANOVA output, then building a between-subjects 2x2 ANOVA table by hand from its sums of squares.

The larger point is that factorial designs let researchers find out what changes the size of an effect, not just whether it exists. A researcher typically starts with one effect of interest, then asks what makes it bigger or smaller, and tests that by adding a second IV.

Each IV in a factorial design gets its own main effect; sometimes that's the effect of interest, but the interaction is usually more informative, since it tests whether IV1's effect changes across levels of IV2. When it does, that's evidence IV2 itself shapes the effect being studied, which is the finding that gets tested and reported. The next chapter picks up exactly there.
