...
Here each condition uses a different set of stream channels, a **between-subjects** design. Had the *same* channels been measured under all four conditions, this identical interaction test could be computed as a paired $t$-test on difference scores, agreeing exactly with a repeated-measures ANOVA ($t^2=F$). That walkthrough lives in the out-of-sequence [meta-narration a nd they don't currently hvae access to that anyway] *Repeated Measures ANOVA* chapter in this book's repository.
::::
...
$SS_\text{Total}$ captures all of the variation in the data: the difference between each channel's score and the grand mean, squared and summed [Are these giant tables necessary in full? Could i just do the first few rows and then like (etc. etc.) and hte final row? It just seems unecessary].
