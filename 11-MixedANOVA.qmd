...
abstract: Portions adapted from the Factorial ANOVA chapter, contributors James Lavender & Alistair Poore. "Environmental Computing" <https://environmentalcomputing.net/>
---
...
In this chapter we go beyond single-factor designs to factorial designs, where two or more independent variables are studied together. Factorials let you test **main effects** (average differences for each factor) and **interactions** (whether the effect of one factor depends on the level of another). That’s the core logic you’ll use for developing hypotheses for ANOVA tests. 
...
No interaction means **parallel lines** or **equal difference scores** across the moderator. If the Pollution effect is 10 at Low Flow and 10 at High Flow, the difference-of-differences is 0, so you fail to reject the interaction.
:::


## Applied Primer

Applied primer for H2)

In a factorial design, you test one null hypothesis per effect you plan to interpret. In a 2×2 there are three effects, so three nulls: two main effects and one interaction.

## How to write hypotheses

Let factor A have levels A1, A2 and factor B have levels B1, B2. Let $\mu_{ij}$ be the mean for cell $(i,j)$. Let the marginal means be

* $\mu_{A1\cdot} = \dfrac{\mu_{11}+\mu_{12}}{2}$ and $\mu_{A2\cdot} = \dfrac{\mu_{21}+\mu_{22}}{2}$
* $\mu_{\cdot B1} = \dfrac{\mu_{11}+\mu_{21}}{2}$ and $\mu_{\cdot B2} = \dfrac{\mu_{12}+\mu_{22}}{2}$

### Main effect of A

* **Null** $H_0$: $\mu_{A1\cdot} = \mu_{A2\cdot}$
* **Alternate** $H_1$: $\mu_{A1\cdot} \ne \mu_{A2\cdot}$

### Main effect of B

* **Null** $H_0$: $\mu_{\cdot B1} = \mu_{\cdot B2}$
* **Alternate** $H_1$: $\mu_{\cdot B1} \ne \mu_{\cdot B2}$

### A×B interaction

There are three equivalent ways to state the **interaction null**. Use whichever is most intuitive for the audience.

1. **Difference-of-differences equals zero**
   $H_0$: $(\mu_{11}-\mu_{12})=(\mu_{21}-\mu_{22})$
   $H_1$: $(\mu_{11}-\mu_{12})\ne(\mu_{21}-\mu_{22})$

2. **Simple effects of A are equal across levels of B**
   $H_0$: $(\mu_{11}-\mu_{21})=(\mu_{12}-\mu_{22})$

3. **Parallel lines** (graphical)
   $H_0$: the A effect has the same size at B1 and B2, so the two lines are parallel; non-parallel lines imply an interaction.

> Plain-English read: under the interaction null, the **effect of one factor does not depend on the level of the other factor**.


## Example

Let A = Pollution (No, Yes). Let B = Flow (Low, High). Let DV = macroinvertebrate diversity.

* **Main effect of Pollution**

  * $H_0$: average diversity with No Pollution equals average diversity with Pollution, collapsing over Flow.
  * $H_1$: they differ.

* **Main effect of Flow**

  * $H_0$: average diversity under Low Flow equals average diversity under High Flow, collapsing over Pollution.
  * $H_1$: they differ.

* **Pollution × Flow interaction**

  * $H_0$: the **Pollution effect** is the same at Low and High Flow.
    In symbols: $(\mu_{\text{No,Low}}-\mu_{\text{Yes,Low}})=(\mu_{\text{No,High}}-\mu_{\text{Yes,High}})$.
  * $H_1$: the Pollution effect differs by Flow.

**Interpretation map**

* Reject Pollution $H_0$ → on average, biodiversity differs between polluted and unpolluted reaches.
* Reject Flow $H_0$ → on average, biodiversity differs between flow regimes.
* Reject interaction $H_0$ → the size or direction of the Pollution effect changes with Flow. This is your “does Flow modulate the Pollution effect?” question.

## Some template hypotheses 

If you are conducting ANOVA, you can use these templates to construct your hypotheses succinctly and consistently.

* **Main effect of A**

  * *Null:* “There is no difference in mean [DV] between A1 and A2.”
  * *Alternate:* “Mean [DV] differs between A1 and A2.”

* **Main effect of B**

  * *Null:* “There is no difference in mean [DV] between B1 and B2.”
  * *Alternate:* “Mean [DV] differs between B1 and B2.”

* **A×B interaction**

  * *Null:* “The effect of A on [DV] is the same at B1 and B2.”
    Optional math: $(\mu_{A1,B1}-\mu_{A2,B1})=(\mu_{A1,B2}-\mu_{A2,B2})$.
  * *Alternate:* “The effect of A on [DV] differs across levels of B.”

**Tip:** If you predict a direction, say so in the alternate.
Example: “We predict a **smaller** Pollution effect under High Flow than Low Flow.”
...
When we conduct a two-way ANOVA, we always begin by testing the **interaction** term.

* If the null hypothesis of *no interaction* is **rejected**, we **do not** interpret the main effects in isolation, because they are not consistent across conditions. Instead, we focus on the simple effects that describe how one factor behaves at each level of the other.
...
Sometimes researchers extend factorial designs to include **three independent variables**, such as irrigation method (drip vs. sprinkler), soil type (sandy vs. clayey), and crop type (wheat vs. corn). A **2×2×2** design like this allows for:

* three **main effects** (one for each factor)
* three **two-way interactions** (A×B, A×C, B×C)
* one **three-way interaction** (A×B×C)

You can think of a 2×2×2 as *two side-by-side 2×2 plots*—one for each level of the third variable.

```{r}
#| label: fig-11222
#| fig-cap: "Example means from a 2×2×2 design shown as two panels, one for each crop type."
#| out-width: "100%"
#| fig-asp: 0.5
...
ggplot(df, aes(x = soil_type, y = crop_yield, color = irrigation, group = irrigation)) +
  geom_point() + geom_line() + theme_classic() + facet_wrap(~ crop_type)
```

A **three-way interaction** means the 2×2 pattern itself changes across the third variable—for instance, the irrigation×soil interaction looks different for wheat than for corn.

That’s really all you need to know: interactions can themselves interact. Beyond this point, designs get exponentially harder to interpret, which is why in practice we usually stop at two factors unless there’s a strong theoretical reason to go further.
