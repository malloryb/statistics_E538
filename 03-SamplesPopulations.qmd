...
These rules look abstract, but each one just formalizes a common-sense situation. Keep the table as a reference; you will not need to manipulate the notation in this course. Two of the four ideas do come back, so they are worth a closer look.

### Conditional probability

$P(A \mid B)$ is read "the probability of $A$ given that $B$ has occurred." Conditioning narrows the sample space: instead of asking how likely $A$ is among all outcomes, you ask how likely it is among the outcomes where $B$ already happened. A stream might exceed a nutrient threshold on 20% of all days, but on 60% of days following heavy rain. Same stream, different question.

This is the shape of every inference from here on. A $p$-value, which arrives in Chapter 5, is a conditional probability: the probability of data at least this extreme *given that* the null hypothesis is true.

### Independence

Two events are **independent** when knowing that one occurred tells you nothing about whether the other did, which is to say that conditioning changes nothing:

$$P(A \mid B) = P(A)$$

That simplifies the "and" rule from the table above. When $A$ and $B$ are independent, their joint probability is just the product:
...
-   Rules of probability: Complements, intersections (and), unions (or), and conditional probabilities all follow from the basic distribution framework. The two that carry forward are **conditional probability**, which is the form every $p$-value takes, and **independence**, which is the assumption underneath every test in this book.
...
We started with a uniform distribution, but @fig-4unifmany does not look uniform at all. The histogram of sample means has its own shape and its own variability, and that fact leads directly to the **central limit theorem**.
...
![Simulation of samples (grey), their means (red line), and the sampling distribution of the mean (blue). As sample size grows, the distribution of sample means narrows and centers on the population mean. (Final frame; the animated version is in the web edition of this book.)](imgs/gifs/stills/sampleDistNormal-1.png){#fig-4samplingmean width="75%"}
:::
...
ggplot(all_df, aes(x=sample))+
  ...
  geom_histogram(aes(x=sample_means,y=(..density..)/max(..density..)),fill="#63b8ff",color="white",alpha=.5)+
  stat_function(fun = dnorm, 
                args = list(mean = 0, sd = 1), 
                lwd = .75, 
                col = 'red')+
  geom_vline(aes(xintercept=sample_mean,frame=sims),color="#b22222")+
...
In @fig-4sampledistmeannorm, we are sampling from a *normal* distribution (red line). The sampling distribution of the mean (blue bars) is also normal.
...
ggplot(all_df, aes(x=sample))+
  geom_histogram(aes(x=sample_means,y=(..density..)/max(..density..)),fill="#63b8ff",color="white",alpha=.5,bins=75)+
  stat_function(fun = dnorm, 
                args = list(mean = 0, sd = 1), 
                lwd = .75, 
                col = 'red')+
...
ggplot(all_df, aes(x=sample))+
  geom_histogram(aes(x=sample_means,y=(..density..)/max(..density..)),fill="#63b8ff",color="white",alpha=.5,bins=75)+
  geom_hline(yintercept=.1,color="#b22222")+
...
Two bear directly on what we just saw:
