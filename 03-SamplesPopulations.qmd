...
Let's try sampling from a uniform population: the parent distribution is flat (red), but the sampling distribution of the mean is bell-shaped (@fig-4samplemeanunif).[Why is base size 20 here, it looks huge?]
...
Sampling from an exponential population: the parent distribution is skewed, but the sampling distribution of the mean still looks normal (@fig-4samplemeanExp). [Thinking these ggtitles are unnecessary]
...
Those same proportions hold for any normal distribution, regardless of its mean or SD. In @fig-4normalSDspercentsB (with $\mu=100,\ \sigma=25$), the region from 100 to 125 (one SD above the mean) still contains about 34.1% of values.[labels actually look fine here at least]
...
[This is raw output, should be kable not tibble it's ugly]. Once standardized, areas under the normal curve tell you how unusual a score is. z between −1 and 1 is common (\~68% of cases), between 1 and 2 is less common (\~13.6% on each side), and beyond ±2 is relatively rare (\~2.1% in each tail segment between 2 and 3, even less beyond 3).
...
### Why variance needs a small correction [can this part cover for what I had in Ch 1? It's stated twice I think = yeah it's in 1.10.5 also]
