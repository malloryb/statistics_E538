...
Chapter 1 worked with **one variable at a time**. It covered the center in full (mean, median, mode) and gave a first, rough measure of spread, the range. Many interesting questions are about whether two variables move together. Does ozone rise on hotter days? Do wetter winters produce taller trees?

The chapter starts with what two related variables look like side by side, in a scatter plot. It then builds the measure of *differentness* that Chapter 1 promised, to replace the **range**. That same measure, extended to two variables, gives us covariance, then Pearson's $r$, a first look at regression, and finally the specific ways a correlation can mislead you.
...
Two other patterns are possible: a reversed relationship, where more chocolate means less happiness, and no relationship at all, where chocolate supply has nothing to do with happiness. @fig-2posnegrand shows all three side by side:
...
Before we can measure how two variables move *together*, we need the proper measure of *differentness* Chapter 1 promised: a number for how much a single variable moves at all. The same calculation, applied to two variables at once, gives covariance, so we build it here and extend it in the next section.
...
The count grows fast, though: 10 numbers means 10 × 10 = 100 comparisons, 500 numbers means 500 × 500 = 250,000. Pairwise comparisons have a second problem: the positive and negative differences cancel out, so their average is zero no matter what the data look like. These problems motivate a much simpler approach: compare each score to one reference point instead, the mean.
...
Chapter 3 explains when to divide by $N$ (when you have the entire population) and when to divide by $n-1$ (when you only have a sample, which is almost always the case). For now, just use $N$.
...
**And now the problem.** Those deviations sum to zero. The positives and negatives cancel exactly, which would suggest there is no variation at all, and that is obviously false. Squaring will fix it, but first we look at *why* the cancellation happens.
...
The mean is the one point where the data balance, which is why the raw deviations always sum to zero. To summarize variation, we need a way around this problem.
...
the_df <- the_df %>%
  rbind(c("Sums",round(colSums(the_df[1:6,2:5]),2))) %>%
  rbind(c("Means",round(colMeans(the_df[1:6,2:5]),2)))
knitr::kable(the_df)
...
The last column shows absolute deviations. If we take their mean, we get the mean absolute deviation (MAD). The MAD acts a lot like the standard deviation: it's the average size of a deviation from the mean, only without squaring. Both approaches, squaring and taking absolute values, solve the same problem in slightly different ways. Squaring is more common because it leads to nice mathematical properties (like in regression and ANOVA), but the MAD can be more intuitive to interpret.
...
We've added a new column called `Chocolate_X_Happiness`, which is each person's chocolate score multiplied by their happiness score. The products show how the two values move together.
...
This is the basis of *covariance*: products of paired values show whether two variables move in the same direction or in opposite directions.
...
the_df_short<-cbind(the_df_short,
                    Cd = round((chocolate[1:10]-mean(chocolate[1:10])),digits=2),
                    Hd = round((happiness[1:10]-mean(happiness[1:10])),digits=2),
...
One note before moving on. The formula above divides by $N$, which is the population version, and that is what the table computes. R's `cov()` and `sd()` divide by $n-1$ instead, for a reason Chapter 3 explains. The two differ only by a factor of $n/(n-1)$, and that factor cancels completely in Pearson's $r$ below, so $r$ comes out identical either way. If you compute the covariance itself in R, though, expect a slightly larger number than the one in this table.

Covariance has one drawback: its magnitude is hard to interpret. Just as variance is tied to squared units, covariance is tied to the scales of both variables, so a covariance of 3.22 isn't inherently "large" or "small" the way a correlation is. All we can read directly from it is the sign, not the strength. To compare strength across variables with different units or scales, we need to normalize covariance, which leads to Pearson's $r$.
...
**Linear regression** is closely tied to correlation, and this section gives a first introduction to it.
...
Each panel now has a red line. Each is a **best fit** line: the single straight line that best summarizes the overall pattern of dots.
...
ggplot(d, aes(x = hp, y = mpg)) +
  geom_smooth(method = "lm", se = FALSE, color = "#10377f") +  # Plot regression slope
...
Whenever the black line does not overlap with the blue line, it fits the data less well, because the blue regression line has the smallest total squared error.
...
@fig-2corwithLine shows a small example: seven $x,y$ pairs, plotted with their fitted line.
...
In Week 11 we compute a line's slope and intercept by hand and use those values to compute and interpret residuals for individual points.
...
Two measures that have nothing to do with one another can still show a positive or negative correlation by random chance, and this happens often. These are called **spurious** correlations.
...
A more useful question is what range of correlations chance alone can produce. If we ran this game thousands of times, each time drawing new balls and computing the correlation, the $r$ value would fluctuate across the whole range. Looking at the shape of that fluctuation tells us what kinds of correlations chance can produce.
...
**How do we know whether the correlation we observed is meaningful or just chance?** One way forward is to look at the whole distribution of outcomes chance produces, rather than at a single outcome. By simulating 1,000 random correlations and plotting them in a histogram, we can see which values occur frequently and which are rare.
...
As sample size grows, the spread of chance correlations narrows:
...
**Larger samples reduce the influence of chance.** Small samples are highly unstable, making it difficult to distinguish signal from noise. As sample size increases, the distribution of chance correlations collapses toward zero, so observing a large $r$ becomes far less likely under randomness alone.
...
With $n=10$, the line swings from steep positive to steep negative slopes. At $n=50$ and $n=100$, the line still shifts but is more restrained. At $n=1000$, the line is consistently flat, as expected when no correlation exists.
...
The line from the sample with 1000 observations is the most reliable. It stays nearly flat across repetitions, as we would expect when no true correlation exists. The line from a sample of only 10 observations swings widely from one repetition to the next.
...
This means no *single straight line* describes the relationship. When we need lines or curves going in more than one direction, we have a nonlinear relationship.
...
Francis Anscombe (@Anscombe1973) made this point with what is now called *Anscombe's Quartet*. Each panel in @fig-anscombe shows pairs of $x$ and $y$ values as a scatterplot.
...
ggplot(ac_long, aes(x=x, y=y))+
  geom_point(size=1.8, color="#10377f")+
  geom_smooth(method="lm", se=FALSE, color="#b22222")+
...
The same issue appears in human data. Suppose we find that, across an elementary school, children with bigger feet read better. Shoe size does not improve reading. **Age** makes feet bigger, and age also improves reading. Comparing shoe size and reading ability without accounting for age would credit shoe size with an effect that age is actually driving.

## Chapter Summary

**Why it matters.** Correlation is how we quantify whether two variables move together. We use it to describe patterns (e.g., temperature and canopy cover, chocolate and happiness) and to decide whether a relationship is worth modeling or acting on.
