...
We've already covered central tendency: mean, median, mode. Variation is a different story. Chapter 1 measured spread with the range, and flagged it as a blunt instrument, a placeholder for something built from every observation, not just the two extremes. That something, variance and standard deviation, is what we build first here, before extending the same idea to how two variables move together. The path there starts with understanding covariance.

## How much does one variable vary?

Before we can measure how two variables move *together*, we need the proper measure of *differentness* Chapter 1 promised: a number for how much a single variable moves at all. That turns out to be the same machinery needed for two variables, used twice, so we build it once here and then extend it on the next page.
