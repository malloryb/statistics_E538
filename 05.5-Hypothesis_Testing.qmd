...
ha_left <- dnorm(x_vals, mean=-4, sd=2)  # Adjusted standard deviation for wider curve
...
p3 <- ggplot() + 
  ...
  annotate("text", x = -2.5, y = 0.25, label = expression(mu >= "X"), size = 5) +
  annotate("text", x = -2.5, y = 0.15, label = expression(mu <= "X"), size = 5) +
...
p4 <- ggplot() + 
  ...
  geom_vline(aes(xintercept=0), linetype="dotted", color="black", size=0.5) +
geom_segment(aes(x = -0.7, xend = 0.7, y = 0, yend = 0), color="darkgreen", size=1.5)+ # Adjusted CI width
