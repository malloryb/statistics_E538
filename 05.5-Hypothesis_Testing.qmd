...
plot_distributions_no_error <- function(null_mean, null_sd, sample_mean, sample_sd, inferred_mean, inferred_sd) {
  ...
  p <- ggplot() + 
    ...
    geom_line(aes(x=x_vals, y=sampling_mean, color="Sampling Distribution of the Sample Mean"), linetype="dotdash", size=1) +
    geom_line(aes(x=x_vals, y=inferred_parent, color="Inferred Parent Distribution"), linetype="dashed", size=1.5) +
    geom_segment(aes(x = null_mean, xend = null_mean, y = y_intersect, yend = 0), linetype="dotted", color="magenta", size=1) +
    geom_segment(aes(x = CI_left, xend = CI_right, y = 0., yend = 0), color="darkgreen", size=1.5) +
    theme_classic() + 
    theme(legend.position='bottom', 
          axis.text = element_blank(), 
...
Let's go back to our pollution example. Imagine in the past, the pollution level averaged 10.0 ppb. We can set our hypotheses as:
...
From the figure below, we've outlined the various elements crucial for hypothesis testing. Think of this section as a handy guide. Whenever you come across detailed graphs later in this chapter, you can circle back here for clarity.
...
theme_set(theme_classic() + 
          theme(legend.position=c(0.8,0.8), axis.text = element_blank(), # Adjusted legend position to be inside the plot
...
p_key <- ggplot() + 
 ...
 theme(legend.position=c(0.2,0.8),  # Centered legend
      legend.text = element_text(size = 12),  # Increased legend text size
      plot.margin = margin(1, 1, 1, 4, "cm")) + # Adjusted plot margins size
  labs(color = NULL)  # Removes the "colour" label from the legend
...
#| out-width: "100%"
#| 
p4 <-  plot_distributions(null_mean=-0.5, null_sd=1, sample_mean=0.6, sample_sd=0.3, inferred_mean=0.6, inferred_sd=1, true_mean=0.5, true_sd=1)
print(p4)
...
p7 <- plot_distributions(null_mean=-0.2, null_sd=1, sample_mean=0.5, sample_sd=0.4, inferred_mean=0.5, inferred_sd=1, true_mean=0.5, true_sd=1)
...
p8 <- plot_distributions(null_mean=-0.5, null_sd=1, sample_mean=2, sample_sd=0.5, inferred_mean=2, inferred_sd=1, true_mean=2, true_sd=1)
print(p8)
```
...
p9 <- plot_distributions(null_mean=-0.5, null_sd=1, sample_mean=1, sample_sd=0.4, inferred_mean=1, inferred_sd=1, true_mean=4, true_sd=1)
print(p9)
```
...
plot_distributions_no_error(null_mean=0, null_sd=1, sample_mean=-0.2, sample_sd=0.7, inferred_mean=-0.2, inferred_sd=1)
...
plot_distributions_no_error(null_mean=0, null_sd=1, sample_mean=-0.5, sample_sd=0.3, inferred_mean=-0.5, inferred_sd=1)
```
