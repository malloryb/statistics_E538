...
#| fig-cap: "Averaging the numeric color codes of four land cover classes returns a color, but not a land cover class. Nothing on the map is that color, and there is no class sitting between forest and urban for it to represent. The arithmetic ran; the answer means nothing."
...
#| fig-cap: "The same three athletes shown two ways. On the podium they are one place apart, and one place apart again, evenly spaced by construction. On the scale below, the weights they actually lifted are 29 kg and 2 kg apart. Ranking preserves the order and discards the distances, which is why the average of a set of ranks does not mean anything."
...
#| fig-cap: "The same physical range of temperatures, from absolute zero up to the boiling point of water, shown on two scales. Both rails cover identical ground; only the placement of zero differs. Celsius anchors its zero to the freezing point of water, leaving a long stretch of perfectly real temperature below it. Kelvin anchors its zero to the point where there is no thermal energy at all, so nothing can sit below it. That difference is what separates an interval scale from a ratio scale."
