...
Environmental questions are usually questions about change. Is this stream warming? Did the restoration work? They are hard to answer because the systems never sit still to begin with. Streams warm and cool over a single day, and vegetation recovers in fits and starts. Something is always changing, so finding a real effect means separating it from the change that would have happened anyway.

That is what statistics is for. It gives you a way to say how much of what you observed is the thing you were looking for and how much is ordinary variation, and how confident you should be in telling the two apart. Most environmental claims you will read or make rest on that distinction, whether or not anyone says so out loud.
...
The reversal happens because Italy's population skews older while China's skews younger, and baseline risk rises steeply with age. Italy had a greater proportion of older people getting sick, which pulls Italy's overall average up. Similarly, China's average is pulled down because a greater proportion of younger people are getting sick. Aggregating without adjusting for age hid the within-group pattern entirely. Once we asked a more specific question, the answer reversed.
...
ggplot(fig3_df, aes(x = Age, y = Prop, fill = Country, group=Country)) +
  ...
  theme_classic(base_size = 13) +
  theme(legend.position = "none", panel.border = element_rect(size = 1, fill=NA), axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
```

Sheer volume is the other reason. Satellites measure surface temperature, field stations log rainfall, and sensors track air quality, so a single study can generate thousands of rows. On top of that, river flow changes by the hour and human activity layers on more variability still. Statistics is how we compress all of that into something we can reason about.
...
Unequal group sizes come back when we reach ANOVA in Chapter 8, where each group mean is weighted by its own $n$ for exactly this reason.
...
When the numbers and the graph disagree, believe the graph. A summary can only report what it was designed to measure, while a plot shows you everything, including whatever you failed to anticipate. The next two examples show how badly summaries can mislead.
