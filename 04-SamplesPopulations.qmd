...
-   Chocolate $P(\text{choc})=0.5$

-   Vanilla $P(\text{van})=0.3$

-   Strawberry $P(\text{straw})=0.2$
...
These are ways of expressing relationships within a distribution. They look complicated but let's quickly walk through them. We'll add a fourth flavor to our ice cream flavors here, Rocky Road. So we have our new sample space $S$, and then we'll consider two non-elementary events $A$ and $B$.

$S = (chocolate, vanilla, strawberry, rocky road)$

$A = (chocolate, vanilla, strawberry)$

$B = (chocolate, rocky road)$

**Not** $A$:

$P(\neg A)$ = $1-P(A)$

This means that the probability of "not $A$" is equal to one minus the probability of $A$.

$A$ **and** $B$:

This statement means that $A$ and $B$ both happen. In other words, the elementary events need to belong to both $A$ and $B$.

In our case, the only ice cream flavor that occurs in both $A$ and $B$ is chocolate.
...
This statement means that at least one of $A$ or $B$ happens. That means your elementary event must belong to $A$, $B$, or both. 

In our case: 

$A \cup B = (chocolate, vanilla, strawberry, rocky road)$

Next, we can assign probabilities to these events. 
