...
*Explanation*: $A \cup B$ means "a flavor in A or in B." In this case, that covers all four possible flavors, i.e., the entire sample space. Therefore, $P(A \cup B) = 1$.
...
This $\hat\sigma^2$ is an unbiased estimator of $\sigma^2$. For standard deviation, we use

$$\hat\sigma = \sqrt{\hat\sigma^2}.$$

Strictly, $\hat\sigma$ is still slightly biased for $\sigma$, but it is conventional and practically preferred.

Practice and terminology

- Software defaults: Some functions divide by $N-1$, others by $N$. Always check. 

Terminology: 

- $s$ (divide by $N$): property of the sample, but biased for $\sigma$. 

- $\hat\sigma$ (divide by $N-1$): estimate of the population SD.

In everyday analysis, most people report $\hat\sigma$ but casually call it the "sample SD." It's fine in practice, but conceptually it's worth keeping the distinction clear: $s$ describes what you observed, while $\hat\sigma$ is your best guess at the population's variability.

::: callout-note
The $N-1$ correction matters most for small samples. When $N$ is large, the difference between dividing by $N$ and $N-1$ becomes negligible, since $N$ and $N-1$ are nearly the same. In practice, the correction only noticeably affects estimates when sample sizes are small.
:::
