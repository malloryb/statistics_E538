...
The one-way ANOVA partitions total variation in the data into two components: variation explained by group membership ($SS_\text{Between}$) and unexplained residual variation ($SS_\text{Within}$). The ratio of the corresponding mean squares, after adjusting for degrees of freedom, produces the F statistic. A large F indicates that between-group differences are large relative to within-group noise.
