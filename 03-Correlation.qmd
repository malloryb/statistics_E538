...
Let’s revisit the idea of variance. Variance tells us how much a set of numbers differs — some values are larger, some are smaller. We’ve seen variance for a single variable, like happiness ratings. Covariance extends this idea to two variables: it describes how their changes relate to each other.

There is variance in chocolate supply across the 10 subjects. We can see that there is variance in happiness across the 10 subjects. We also saw in the scatter plot, that happiness increases as chocolate supply increases; which is a positive relationship, a positive correlation. What does this have to do with variance? Well, it means there is a relationship between the variance in chocolate supply, and the variance in happiness levels. The two measures vary together don't they? When we have two measures that vary together, they are like a happy couple who share their variance. This is what co-variance refers to, the idea that the pattern of varying numbers in one measure is shared by the pattern of varying numbers in another measure.

**Co-variance** is very important. It underlies correlation and many other statistical tools. At first the concept may feel abstract, especially if variance itself still feels new. But working with it repeatedly will build the intuition you need for understanding relationships in data.


::: callout-note
**Tip**: You can think of covariance with a three-legged race. When two people move forward together, their steps are aligned - a positive relationship. If one moves forward while the other moves back, their movements oppose each other - a negative relationship. If each person moves randomly without coordination, there’s still variation, but no consistent relationship between them.
:::

## Turning the numbers into a measure of co-variance

If co-variance is just another word for correlation or relationship between two measures,then, we need some way to measure that. 
...
We've added a new column called `Chocolate_X_Happiness`, which translates to Chocolate scores multiplied by Happiness scores. Each row in the new column, is the product, or multiplication of the chocolate and happiness score for that row. But why would we do this?

Last chapter we reviewed division; now let’s do the same with multiplication. Multiplication means repeated addition:

$2*2= 2+2=4$

$2*6= 2+2+2+2+2+2 = 12$, or $6+6=12$, same thing.

You already know this. The key idea is that multiplication lets us see how two values vary together. Large-with-large or small-with-small pairs produce different products than mixed pairs, and this difference underlies covariance.

So let’s think about multiplying sets of numbers. Two small numbers yield a small product, two large numbers yield a large product, and a small-by-large product falls somewhere in between.

We just saw that multiplying two values together gives us a product. Now let’s extend that to *sets* of values.
...
the_df_short <- the_df_short %>%
  rbind(c("Sums", colSums(the_df_short[1:10, ]))) %>%
  rbind(c("Means", colMeans(the_df_short[1:10, ])))
...
Now, we can get a little bit more formal. In statistics, **co-variance** is not just the straight multiplication of values in X and Y. Instead, it's the multiplication of the deviations in X from the mean of X, and the deviation in Y from the mean of Y. 

Remember those difference scores from the mean we introduced last chapter? We’re going to use them again here, this time to build a measure of how two variables vary together.
...
Notice anything about these blue lines? Hopefully you can see, at least for the first two panels, that they go straight through the data. We call these lines **best fit** lines, because according to our definition (soon we promise) there are no other lines that you could draw that would do a better job of going straight throw the data.
...
::: callout-note
It’s worth noting that many formulas in statistics were developed for convenience in hand calculations, before computers were available. Today we rely on software for computation, but seeing one worked example by hand helps illustrate the logic behind the formulas.
:::

Next we’ll look at the formulas for calculating the slope and intercept of a regression line, and work through one example by hand. The goal is not to memorize the algebra, but to see how the pieces fit together before we shift to using software.
