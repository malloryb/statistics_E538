...
3. **Independent-samples t-test.** Compare means from two groups measured on different units. Use Welch’s test by default when variances differ.
...
df_vals <- tibble(
  dist = c("z (standard normal)", "t (df=25)", "t (df=2)"),
  df    = c(NA, 25, 2)   # NA for z
)
...
Remember, if we obtained a single $t$ from one sample we collected, we could consult the chance window in @fig-7tdecision below to find out whether the $t$ we obtained from the sample was likely or unlikely to occur by chance.
...
**Design in this week’s lab (Mehr, Song, & Spelke, 2016).**
Infants viewed two unfamiliar singers. At **baseline**, infants saw silent videos and researchers recorded the **proportion of looking time to the singer who would later sing the familiar song**. At **test**, after each singer performed, infants again watched silent videos and proportion looking was recorded. We care whether infants **shifted attention** toward the familiar-song singer from baseline to test.

**Define differences and their meaning.**
Choose an order and stick to it:

* (d_i = \text{Test}_i - \text{Baseline}_i)
* (d_i>0): infant looked **more** at the familiar-song singer at test (increase)
* (d_i<0): infant looked **less** at the familiar-song singer at test (decrease)

This subtraction **cancels between-infant variability** (some babies look around more than others) and focuses on **within-infant change**.

**Test on differences (paired (t)).**
Compute one difference per infant, then run a **one-sample (t)** against 0:
...
**Mini walk-through (first 5 infants).**
Using the first five rows you’ll see in lab, suppose
(\bar d = 0.054) and (s_d = 0.168). Then
$$
...
**Full sample effect.**
With all infants (e.g., (n=32)), (\bar d) is similar but (s_d/\sqrt{n}) gets smaller, so (t) increases (in the paper’s data, it’s significant). This is the payoff of pairing: more power from the same number of participants.
...
* Compute differences: `d <- Test - Baseline`
* Paired (t): `t.test(d, mu = 0)` or `t.test(Test, Baseline, paired = TRUE)`
