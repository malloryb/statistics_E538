...
::: callout-tip
**Two reasons people reach for ANCOVA.** If you look this test up elsewhere, you'll often see it introduced differently than we're introducing it here, so it's worth knowing both versions exist:
...
Let's start with the response variable, **bird density**, in @fig-12birddensity. This gives us a quick visual sense of whether urban and rural areas differ in their average values.

```{r}
#| label: fig-12birddensity
...
A scatter plot with fitted regression lines (@fig-12ancovalines) lets us see how air pollution relates to bird density for each area type.

```{r}
#| label: fig-12ancovalines
#| fig-cap: "Bird density against air pollution, with a separate fitted line for each area type. The slopes run in opposite directions, which is what the interaction term tests."
#| fig-alt: "Scatter plot of bird density against air pollution, colored by area type, with separate straight fitted lines for rural and urban sites running in opposite directions."
...
You'll also notice quite a bit of scatter around each line, which makes sense, since our overall $R^2$ is only **0.116**.
...
-   **AirPollution_c (-0.04):** Slope for rural sites, where bird density decreases slightly as pollution increases.
