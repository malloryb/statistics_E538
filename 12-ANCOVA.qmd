...
1. **Omnibus (adjusted means) hypothesis**
   After adjusting for air pollution, do urban and rural sites differ in mean bird density?

   $H_0: \mu^*_{\text{Urban}} = \mu^**{\text{Rural}}$
   $H_A: \mu^**{\text{Urban}} \neq \mu^*_{\text{Rural}}$

   Here, $\mu^*_{\text{Urban}}$ and $\mu^*_{\text{Rural}}$ are the adjusted group means once the covariate (air pollution) is accounted for.

2. **Slope (interaction) hypothesis**
   ...
   A significant interaction means the slopes differ—air pollution’s relationship with bird density depends on area type.

3. **Intercept (baseline difference) hypothesis**
   If the interaction is not significant (slopes are parallel), we can test whether the intercepts differ—i.e., whether the baseline bird density differs between urban and rural areas at zero (or mean-centered) pollution.
